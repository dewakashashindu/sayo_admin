...
export async function GET() {
  try {
    const [recipes, menuItems, rawItems, units, subUnits, locations] = await Promise.all([
      ...
      // Raw / inventory items
      prisma.tbl_ItemMaster.findMany({
        where: { ServiceItem: false, Enable: true },
        select: { ItemCode: true, ItemDes: true, MasterUnitID: true, RawCost: true },
      }),
    ...
    return NextResponse.json({
      ...
      menuItems: menuItems.map(m => ({ code: m.ItemCode.trim(), des: m.ItemDes })),
      rawItems: rawItems.map(r => ({ code: r.ItemCode.trim(), des: r.ItemDes, unit: r.MasterUnitID.trim(), cost: r.RawCost })),
    ...
    return NextResponse.json({ success: false, message: 'Failed to fetch recipes data' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Array.isArray(lines) || lines.length === 0) {
      return NextResponse.json({ success: false, message: 'At least one ingredient line is required' }, { status: 400 });
    }

    const trimmedMenuID = menuItmID.trim();

    // Use transaction to replace existing recipe lines for this menu item
    await prisma.$transaction([
      prisma.tbl_Recipes.deleteMany({
        where: { MenuItmID: trimmedMenuID },
      }),
      prisma.tbl_Recipes.createMany({
        data: lines.map((l: { rowItemCode: string; masterUnitID?: string; subUnitID?: string; qty?: number; locCode?: string; itemCost?: number }) => ({
          MenuItmID:    trimmedMenuID,
          RowItemCode:  l.rowItemCode.trim(),
          MasterUnitID: l.masterUnitID?.trim() || ' ',
          SubUnitID:    l.subUnitID?.trim()    || ' ',
          Qty:          Number(l.qty ?? 0),
          LocCode:      l.locCode?.trim()      || '01',
