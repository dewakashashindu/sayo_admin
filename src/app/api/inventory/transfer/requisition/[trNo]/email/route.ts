// POST /api/inventory/transfer/requisition/:trNo/email
// body: { fromLocCode, toLoc, copy:"standard"|"supplier", to, subject, message }
// Sends Transfer Requisition Note as PDF (same sheet as Print)
...
export async function POST(req:NextRequest, ctx:Ctx){
  const tag="POST /api/inventory/transfer/requisition/[trNo]/email";
  try{
    ...
    if(!to) throw new InvError("Type the address to send this requisition to (To).",400);
    if(!/^[^@\s]+@[^@\s]+\.[^@\s]{2,}$/.test(to)) throw new InvError(`"${to}" is not an e-mail address.`,400);

    const heads=await prisma.$queryRaw<{FromLocCode:string;ToLoc:string;TRNO:string;TRDate:Date|null;TRDueDate:Date|null;NetTotal:number|null;Remarks:string|null}[]>`
      SELECT RTRIM(FromLocCode) AS FromLocCode, RTRIM(ToLoc) AS ToLoc, RTRIM(TRNO) AS TRNO, TRDate, TRDueDate, NetTotal, Remarks
      FROM tbl_transferreqheder WHERE ${keySql("FromLocCode")}=${keyVal(fromLocCode)} AND ${keySql("ToLoc")}=${keyVal(toLoc)} AND ${keySql("TRNO")}=${keyVal(trNo)} LIMIT 1
    `;
    if(!heads.length) throw new InvError(`Transfer Requisition ${trNo} not found (${fromLocCode}→${toLoc}).`,404);
    const head=heads[0];
    const lineRows=await prisma.$queryRaw<{ItemCode:string;ItemDes:string|null;UnitID:string;UnitDes:string|null;CostPrice:number|null;TRQty:number|null}[]>`
      SELECT RTRIM(d.ItemCode) AS ItemCode,
        ...
        d.CostPrice AS CostPrice, d.TRQty AS TRQty
      FROM tbl_transferreqdetail d
      WHERE ${keySql("d.FromLocCode")}=${keyVal(fromLocCode)} AND ${keySql("d.ToLoc")}=${keyVal(toLoc)} AND ${keySql("d.TRNo")}=${keyVal(trNo)}
      ORDER BY d.ItemCode
    `;
    if(!lineRows.length) throw new InvError(`Requisition ${trNo} has no lines, nothing to email.`,400);
    ...
    const lines=lineRows.map(l=>({itemCode:trim(l.ItemCode), name:trim(l.ItemDes)||trim(l.ItemCode), unitID:trim(l.UnitID), unitName:trim(l.UnitDes), costPrice:Number(l.CostPrice||0), poQty:Number(l.TRQty||0)}));
    ...
    const pdf=await buildTransferPdf({
      kind:"requisition", title:"Transfer Requisition Note", copy, copyLabel: poPrintCopyLabel(copy),
      ...
      docNo: trNo, docDate: poPrintDate(head.TRDate), dueDate: poPrintDate(head.TRDueDate),
      issueRef: "", printDate: clock.date, printTime: clock.time, user: actor.name||actor.userId,
    ...
    const fileName=transferPdfFileName(trNo);
    const subject=trim(body.subject)|| transferEmailSubject("requisition", trNo, companyName);
    const text=trim(body.message)|| transferEmailBody({kind:"requisition",docNo:trNo,companyName,from:trim(loc?.LocDes)||fromLocCode,to:trim(locTo?.LocDes)||toLoc,docDate:poPrintDate(head.TRDate),dueDate:poPrintDate(head.TRDueDate),lineCount:rows.length,copy});
    ...
    await logActivity(actor.name,"inventory",`Transfer Requisition ${trNo} emailed to <${to}> as ${fileName} (${poPrintCopyLabel(copy)})`);
    return NextResponse.json({success:true,data:{trNo,fromLocCode,toLoc,to,copy,fileName,bytes:pdf.length},message:`Transfer Requisition ${trNo} emailed to <${to}> — ${poPrintCopyLabel(copy)} attached as ${fileName}.`});
