...
//  Server-side Cloudinary upload for staff profile photos.
//
//  POST  multipart/form-data
//    file       : File  (required)  — JPEG / PNG / WebP / GIF, max 5 MB
//    staffName  : string (optional) — used to build a clean public_id
//
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!cloudName || !apiKey || !apiSecret) {
      console.error('[upload-staff-photo] Missing Cloudinary env vars');
      return NextResponse.json(
    ...
    /* ── Build public_id ── */
    const slug      = (staffName ?? 'staff')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '');
    const publicId  = `sayo/staff/${slug}-${Date.now()}`;

    /* ── Cloudinary signed-upload params ── */
    // Transformation: auto-crop to a 600×600 face-centred square, auto quality
    const transformation = 'c_fill,g_face,h_600,q_auto,w_600';
    ...
    if (!uploadRes.ok || uploadJson.error) {
      console.error('[upload-staff-photo] Cloudinary error:', uploadJson);
      return NextResponse.json(
    ...
    console.error('[upload-staff-photo] Unexpected error:', err);
    return NextResponse.json(
