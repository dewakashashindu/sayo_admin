          ...
          {tab === 'details' && (
            <div className="po-card">
              ...
              <div className="po-actions no-print">
                ...
                <button className="btn" onClick={() => recNo.trim()? setPrintJob({at:new Date()}): showToast('Save first, then print',true)} disabled={busy||!recNo.trim()}>Print</button>
              ...
              {printJob && <InventoryPrintSheet title="Stock Reconciliation Note" docNo={recNo.trim()} docDate={new Date().toLocaleDateString()} printDate={printJob.at.toLocaleDateString()} printTime={printJob.at.toLocaleTimeString()} user="admin" companyName={company.name} companyAddress={company.address} companyPhone={company.phone} branch={locDes} partnerLabel="Location" partnerCode={locCode} partnerName={locDes} partnerAddress={locations.find(l=>l.code===locCode)?.address||''} columns={{code:'ItemCode',des:'Item Description',unit:'Unit',qty:'Phy Qty',cost:'Cost Price',value:'Item Value'}} rows={printRows} totalLabel="Net Value" total={money(netValue)} remarks={remarks} />}
