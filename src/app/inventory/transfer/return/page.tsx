...
// TRANSFER RETURN NOTE — UI only (galapena PO wage)
// Legacy: Transfer Note No / From Location / Transfer Return No / Transfer Return Date
//         To Location / Transfer Note Date / Item Code | Item Name | Unit | Cost Price | TN QTY | Returned QTY | Item Value
...
interface TrLine { key:string; itemCode:string; itemName:string; unitID:string; costPrice:string; tnQty:string; retQty:string; itemValue:number; }
interface FindRow { fromCode:string; fromDes:string; toCode:string; toDes:string; trNo:string; trDate:string; }

let seq=0;
const newLine=():TrLine=>({key:`T${++seq}`,itemCode:'',itemName:'',unitID:'',costPrice:'',tnQty:'',retQty:'',itemValue:0});
...
export default function TransferReturnPage(){
  ...
  const [transferNoteNo,setTransferNoteNo]=useState('TN000012');
  const [fromLoc,setFromLoc]=useState('KITCHEN');
  const [retNo,setRetNo]=useState('TR000003');
  const [retDate,setRetDate]=useState('2026-09-18');
  const [toLoc,setToLoc]=useState('MILLA MIRISSA');
  const [tnDate,setTnDate]=useState('2026-09-21');
  ...
  const loadList = async()=>{
    try{
      ...
      const res=await fetch(`/api/inventory/transfer/return?${params.toString()}`,{cache:'no-store'});
      const j=await res.json() as any;
      if(res.ok && j?.success && Array.isArray(j.data)){
        setFindRows(j.data.map((r:any)=>({fromCode:r.fromLocCode, fromDes:r.fromLocDes||r.fromLocCode, toCode:r.toLoc, toDes:r.toLocDes||r.toLoc, trNo:r.trtnNo, trDate:String(r.trtnDate||'')} )));
      }
    }catch{ setFindRows([]); }
  };
  useEffect(()=>{ if(tab==='find') void loadList(); },[tab, findStatus]);

  const [openTNs,setOpenTNs]=useState<string[]>([]);
  useEffect(()=>{ let a=true; (async()=>{ try{ const r=await fetch('/api/inventory/transfer/note?status=confirmed&limit=100',{cache:'no-store'}); const j=await r.json() as any; if(a && r.ok && j?.success && Array.isArray(j.data)){ setOpenTNs(j.data.map((x:any)=> String(x.tranNo).trim()).filter(Boolean)); } }catch{} })(); return()=>{a=false}; },[]);
  useEffect(()=>{ let a=true; (async()=>{ try{ const r=await fetch('/api/inventory/lookups',{cache:'no-store'}); const j=await r.json() as any; if(!a) return; if(r.ok&&j?.success){ const locs=j.locations??[]; if(locs.length){ setLocations(locs); setUnits(j.units??[]); if(j.company?.name) setCompany(j.company); setLookupNote(`${locs.length} location(s) loaded`); } else throw new Error('empty'); } else throw new Error('fail'); } catch{ if(!a) return; setLocations([{code:'KITCHEN',des:'KITCHEN',address:'',enable:true},{code:'01',des:'MILLA MIRISSA',address:'',enable:true},{code:'03',des:'KITCHEN',address:'',enable:true}]); setUnits([{id:'KILOGRAM',des:'KILOGRAM',enable:true},{id:'KILOGRAN',des:'KILOGRAM',enable:true}]); setLookupNote('Demo locations — API not reachable'); } })(); return()=>{a=false}; },[]);

  const netValue = useMemo(()=> lines.reduce((s,l)=> s+ (Number(l.costPrice)||0)*(Number(l.tnQty)||0),0),[lines]);
  function patchLine(k:string,p:Partial<TrLine>){ setLines(prev=> prev.map(l=>{ if(l.key!==k) return l; const n={...l,...p} as TrLine; n.itemValue=(Number(n.costPrice)||0)*(Number(n.tnQty)||0); return n; })); setDirty(true); }
  ...
  async function handleSave(confirm=false){
    ...
    if(!transferNoteNo){ show('Choose Transfer Note No', true); return; }
    const valid = lines.filter(l=> (l.itemCode||'').trim() || (l.itemName||'').trim());
    if(valid.length===0){ show('Add at least one item', true); return; }
    try{
      const body = { tnNo: transferNoteNo, fromLocCode: fromLoc, toLoc: toLoc, trRtnDate: retDate, remarks, confirm, lines: valid.map(l=>({ itemCode:l.itemCode||l.itemName, unitID:l.unitID, costPrice: Number(l.costPrice)||0, tnQty: Number(l.tnQty)||0, tranRtnQty: Number(l.retQty)||0 })) };
      const res = await fetch('/api/inventory/transfer/return',{method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
      const j = await res.json() as any;
      if(!res.ok || !j?.success) throw new Error(j?.message||'Save failed');
      setRetNo(j.data.trtnNo); setConfirmed(j.data.confirmed); setDirty(false); show(j.message||'Saved ✓');
      void loadList();
  ...
  function handlePrint(){
    ...
    if(!retNo){ show('Save the return before printing', true); return; }
    setPrintAsk(true);
  }
  ...
  const printData = (()=> {
    ...
    return {
      ...
      docNo: retNo || '(not saved)',
      docDate: poPrintDate(retDate),
      issueRef: transferNoteNo,
      printDate: poPrintClock(at).date,
      printTime: poPrintClock(at).time,
      user: 'aura',
      rows: poPrintRows(printableLines.map(l=>({itemCode:l.itemCode, name:l.itemName, unitID:l.unitID, unitName: units.find(u=>u.id===l.unitID)?.des||l.unitID, costPrice:Number(l.costPrice)||0, poQty:Number(l.retQty)||0}))),
      total: poPrintTotal(printableLines.map(l=>({itemCode:l.itemCode,name:l.itemName,unitID:l.unitID,costPrice:Number(l.costPrice)||0, poQty:Number(l.retQty)||0}))),
      remarks: (remarks||'').trim(),
    };
  })();
  function openMailDialog(){
    if(printableLines.length===0){ show('Add at least one item before emailing', true); return; }
    if(!retNo){ show('Save the return first (Save), then it can be emailed', true); return; }
    setMailTo(prev=> prev || '');
  ...
  async function sendMail(){
    ...
    setSending(true);
    try{
      const res=await fetch(`/api/inventory/transfer/return/${encodeURIComponent(retNo)}/email`,{
  ...
  const filteredFind = findRows.filter(r=> !findQ || r.trNo.includes(findQ));
  ...
  return (
    <>
      ...
      <div className="po-shell">
        <div className="no-print"><AdminSidebar active="inv-tr-ret" onNav={handleNav} onLogout={()=>router.push('/admin-login')} /></div>
        <div className="po-main">
          <header className="po-head no-print">
            <h1>TRANSFER RETURN NOTE</h1>
            ...
            <div className="po-state">
              <span className="chip">{retNo || 'new'}</span>
          ...
          {lookupNote && <div className="po-note no-print">{lookupNote} — Transfer Return (TR) like legacy page 20</div>}

          {tab==='find' && (
            <div className="po-card no-print">
              <div className="po-list-wrap">
                <table className="po-table">
                  <thead><tr><th>FromLocCode</th><th>LocDes</th><th>ToLoc</th><th>ToLocDes</th><th>TRtnNO</th><th>TRtnDate</th></tr></thead>
                  <tbody>
                    {filteredFind.map(r=>(
                      <tr key={r.trNo} style={{background: r.trNo==='TR000002' ? '#eaffea' : '#fff8dc',cursor:'pointer'}} onClick={()=>{ setRetNo(r.trNo); setTab('details'); show(`Return ${r.trNo} selected`); }}>
              ...
              <div className="po-find">
                ...
                <input value={findQ} onChange={e=>setFindQ(e.target.value)} placeholder="TR number or location" />
              ...
              <div className="po-note" style={{background:'#fff',border:'1px solid #000',padding:10}}>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:'11px'}}>
                  ...
                  <div style={{fontWeight:800}}>Transfer Return Note</div>
                  <div style={{fontSize:'10px',textAlign:'right'}}>Print Date : 21-Sep-2026<br/>Print Time : 9:55:21 am<br/>User : aura</div>
                </div>
                <div style={{display:'flex',gap:30,fontSize:'11px',marginTop:10}}><span>From <b>{fromLoc || '—'}</b> Tra Rtn Date {retDate}</span><span>To <b>{toLoc || '—'}</b> Txn Date {tnDate}</span></div>
                <div style={{display:'flex',gap:30,fontSize:'11px'}}><span>Transfer Return No <b>{retNo || 'TR —'}</b> Transfer Note <b>{transferNoteNo || 'TN —'}</b></span><span>Tra. User <b>aura</b></span></div>
                <table className="po-table" style={{marginTop:8}}>
                  <thead><tr><th>Item Code</th><th>Item Description</th><th>Unit</th><th className="num">Cost Price</th><th className="num">Tran Rtn Qty</th><th className="num">Item Value</th></tr></thead>
                  <tbody>
                    {lines.filter(l=>l.itemCode||l.itemName).length===0 ? <tr><td colSpan={6} className="empty">No lines — select a return from above or add in Details</td></tr> : lines.filter(l=>l.itemCode||l.itemName).map(l=>(
                      <tr key={l.key}><td>{l.itemCode || '—'}</td><td>{l.itemName || '—'}</td><td>{l.unitID || '—'}</td><td className="num">{l.costPrice ? Number(l.costPrice).toLocaleString(): '—'}</td><td className="num">{l.retQty || '—'}</td><td className="num">{Number(l.itemValue||0).toLocaleString()}</td></tr>
                    ))}
                    <tr><td colSpan={5} style={{textAlign:'right',fontWeight:700}}>Total</td><td className="num" style={{fontWeight:800}}>{lines.reduce((s,l)=> s+Number(l.itemValue||0),0).toLocaleString()}</td></tr>
                  </tbody>
                </table>
              </div>
            </div>
          ...
          {tab==='details' && (
            <div className="po-card">
              <div className="po-form no-print" style={{gridTemplateColumns:'140px minmax(150px,1fr) 110px minmax(150px,1fr) 140px minmax(150px,1fr)'}}>
                <label>Transfer Note No</label>
                <select value={transferNoteNo} onChange={e=>setTransferNoteNo(e.target.value)}>
                  {openTNs.length ? openTNs.map(tn=> <option key={tn} value={tn}>{tn}</option>) : <><option>TN000012</option><option>TN000011</option><option>TN000010</option></>}
                </select>
                ...
                <label>Transfer Return No</label>
                <input value={retNo} readOnly className="mono" style={{background:'#fff8dc',fontWeight:700}} />

                <label>Transfer Return Date</label>
                <input type="date" value={retDate} onChange={e=>{setRetDate(e.target.value); setDirty(true);}} />
                ...
                <label>Transfer Note Date</label>
                <input type="date" value={tnDate} disabled style={{background:'#eef2f2'}} />
              </div>
                ...
                <table className="po-table">
                  <thead>
                    <tr>
                      ...
                      <th style={{width:90}} className="num">TN QTY</th>
                      <th style={{width:110}} className="num">Returned QTY</th>
                  ...
                  <tbody>
                    {lines.map((l,i)=>(
                      <tr key={l.key} style={i===2?{background:'#2244ff'}:{}}>
                        <td className="num" style={i===2?{background:'#2244ff',color:'#fff'}:{}}>{i===lines.length?'*':'▶'}</td>
                        <td style={i===2?{background:'#2244ff',color:'#fff'}:{}}>{l.itemCode}</td>
                        <td style={i===2?{background:'#2244ff',color:'#fff'}:{}}><ItemSuggestInput locCode={fromLoc} value={l.itemName} onText={t=>patchLine(l.key,{itemName:t})} onPick={it=>pickItem(l.key,it)} /></td>
                        <td><select value={l.unitID} onChange={e=>patchLine(l.key,{unitID:e.target.value})} style={i===2?{background:'#2244ff',color:'#fff'}:{}}><option value="">—</option>{units.map(u=><option key={u.id} value={u.id}>{u.des}</option>)} {!units.find(u=>u.id===l.unitID) && l.unitID && <option value={l.unitID}>{l.unitID}</option>}</select></td>
                        <td className="num"><input type="number" className="num" value={l.costPrice} onChange={e=>patchLine(l.key,{costPrice:e.target.value})} style={i===2?{background:'#2244ff',color:'#fff',textAlign:'right'}:{}} /></td>
                        <td className="num"><input type="number" className="num" value={l.tnQty} disabled style={{background:'#eef2f2',textAlign:'right'}} /></td>
                        <td className="num" style={{background:'#fffde6'}}><input type="number" className="num" value={l.retQty} onChange={e=>patchLine(l.key,{retQty:e.target.value})} style={{background:'#fffde6'}} /></td>
                        <td className="num" style={i===2?{background:'#2244ff',color:'#fff'}:{}}>{money(l.itemValue)}</td>
                        <td className="no-print"><button className="x" onClick={()=>removeLine(l.key)}>✕</button></td>
                      </tr>
                    ))}
                    <tr style={{background:'#2244ff',color:'#fff'}}><td className="num" style={{background:'#2244ff',color:'#fff'}}>*</td><td style={{background:'#2244ff'}}></td><td style={{background:'#2244ff'}}></td><td style={{background:'#2244ff'}}></td><td style={{background:'#2244ff'}}></td><td style={{background:'#2244ff'}}></td><td style={{background:'#2244ff'}}></td><td style={{background:'#2244ff'}}></td><td style={{background:'#2244ff'}}></td></tr>
                  </tbody>
                </table>
              </div>
      ...
      {mailAsk && (
        <div className="ask-back" onClick={()=>setMailAsk(false)}>
          <div className="ask-card mail-card" onClick={e=>e.stopPropagation()}>
            <h2>Email Transfer Return {retNo}</h2>
            ...
            <div className="mail-attach">Attachment: {retNo||'transfer'}.pdf — {printData.rows.length} line(s)</div>
