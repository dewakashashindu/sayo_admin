...
interface TrLine {
  ...
  unitID: string;
  costPrice: string;
  tnQty: string;   // what the transfer note moved (read-only on the grid)
  retQty: string;  // what comes back — the editable quantity
}

interface FindRow {
  trtnNo: string; fromLocCode: string; fromLocDes: string; toLoc: string; toLocDes: string;
  trtnDate: string; tnNo: string; netTotal: number; confirmed: boolean;
}
...
export default function TransferReturnPage() {
  ...
  const [retDate, setRetDate] = useState(dayOf(new Date().toISOString()));
  ...
  /* confirmed transfer notes the return can be written against */
  ...
  const [tnLoading, setTnLoading] = useState(false);
    ...
    (async () => {
      try {
        const res = await fetch('/api/inventory/transfer/note?status=confirmed&limit=200', { cache: 'no-store' });
        const json = await res.json() as { success?: boolean; data?: any[] };
        if (!active || !res.ok || !json?.success) return;
        setTnOptions((json.data ?? []).map((r: any) => ({
          tranNo: String(r.tranNo ?? '').trim(),
    ...
    try {
      ...
      if (!res.ok || !json?.success) throw new Error(json?.message || 'Could not load the transfer returns');
      setList((json.data ?? []).map((r: any) => ({
        trtnNo: String(r.trtnNo ?? ''),
        ...
        trtnDate: dayOf(r.trtnDate),
        tnNo: String(r.tnNo ?? ''),
        netTotal: Number(r.netTotal || 0),
        confirmed: Boolean(r.confirmed),
      })));
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'Could not load the transfer returns', true);
      setList([]);
    } finally {
      setListBusy(false);
    }
  ...
  const unitName = useCallback(
    ...
    if (confirmed) {
      showToast(`${retNo || 'This return'} is already confirmed — Clear to start a new one first`, true);
      return;
    }
    const opt = tnOptions.find((r) => r.tranNo === no);
    const typed = lines.filter((l) => l.itemCode || l.name.trim());
    if (typed.length > 0) {
      if (!confirm(`Loading transfer note ${no} will replace the ${typed.length} line(s) on the form. Continue?`)) return;
    }
    setTnLoading(true);
    try {
      ...
      const res = await fetch(`/api/inventory/transfer/note/${encodeURIComponent(no)}${params.toString() ? `?${params.toString()}` : ''}`, { cache: 'no-store' });
      const json = await res.json() as {
        success?: boolean; message?: string;
        data?: {
          header: { fromLocCode: string; toLoc: string; traDate: string };
          lines: { itemCode: string; itemName: string; unitID: string; costPrice: number; tranQty: number }[];
        };
      };
      if (!res.ok || !json?.success || !json.data) throw new Error(json?.message || 'Could not load the transfer note');
      const h = json.data.header;
      setTnDate(dayOf(h.traDate));
      /* goods come back the way they went: the note's To is the return's From */
      setFromLoc(h.toLoc);
      ...
      setLines(json.data.lines.length
        ? json.data.lines.map((l) => ({
            ...
            tnQty: String(l.tranQty ?? ''),
            retQty: String(l.tranQty ?? ''),
          }))
        : [newLine()]);
      setDirty(true);
      showToast(`Transfer note ${no} — ${json.data.lines.length} item(s) put on the return; direction reversed`);
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'Could not load the transfer note', true);
    } finally {
      setTnLoading(false);
    }
  }
    ...
    try {
      ...
      const json = await res.json() as {
        success?: boolean; message?: string;
        data?: {
          header: { trtnNo: string; fromLocCode: string; toLoc: string; trtnDate: string; tnNo: string; tnDate: string | null; remarks: string; confirmed: boolean };
          lines: { itemCode: string; itemName: string; unitID: string; costPrice: number; tnQty: number; retQty: number }[];
        };
      };
      if (!res.ok || !json?.success || !json.data) throw new Error(json?.message || 'Could not open the transfer return');
      const h = json.data.header;
      setTnNo(h.tnNo || '');
      setTnDate(dayOf(h.tnDate));
      setFromLoc(h.fromLocCode);
      setToLoc(h.toLoc);
      setRetDate(dayOf(h.trtnDate));
      ...
      setLines(json.data.lines.length
        ? json.data.lines.map((l) => ({
            ...
            unitID: l.unitID,
            costPrice: String(l.costPrice ?? ''),
            tnQty: String(l.tnQty ?? ''),
            retQty: String(l.retQty ?? ''),
          }))
        : [newLine()]);
      setDirty(false);
      setLastResult('');
      setTab('details');
      showToast(`Transfer Return ${h.trtnNo} loaded`);
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'Could not open the transfer return', true);
    }
  }
    ...
    return {
      ...
      lines: lines
        ...
        .map((l) => ({
          ...
          tnQty: l.tnQty === '' ? 0 : Number(l.tnQty),
          tranRtnQty: l.retQty === '' ? 0 : Number(l.retQty),
        })),
    };
  }

  async function handleSave(): Promise<boolean> {
    if (!tnNo) { showToast('Choose the Transfer Note No first', true); return false; }
    ...
    if (body.lines.some((l) => !(Number(l.tnQty) > 0))) { showToast('TN QTY is missing on a line — load the items from the transfer note', true); return false; }
    if (body.lines.some((l) => !(Number(l.tranRtnQty) > 0))) { showToast('Enter Returned QTY for each line', true); return false; }

    setSaving(true);
    try {
      const res = retNo
        ? await fetch(`/api/inventory/transfer/return/${encodeURIComponent(retNo)}?fromLoc=${encodeURIComponent(fromLoc)}&toLoc=${encodeURIComponent(toLoc)}`, {
            method: 'PUT', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ tnNo: body.tnNo, trRtnDate: body.trRtnDate, remarks: body.remarks, lines: body.lines }),
          })
        : await fetch('/api/inventory/transfer/return', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...body, confirm: false }),
          });
      const json = await res.json() as {
        success?: boolean; message?: string; hint?: string; data?: { trtnNo: string; netTotal: number };
      };
      if (!res.ok || !json?.success) {
        throw new Error(json?.hint ? `${json.message} — ${json.hint}` : (json?.message || 'Save failed'));
      }
      if (json.data?.trtnNo) setRetNo(json.data.trtnNo);
      setDirty(false);
    ...
    try {
      ...
      const number = retNo || '';
      if (!number) { showToast('Save the transfer return before confirming it', true); return; }
      if (!confirm(`Confirm transfer return ${number}?\nA confirmed return can no longer be edited or deleted.`)) return;

      const res = await fetch(`/api/inventory/transfer/return/${encodeURIComponent(number)}/confirm`, {
        method: 'POST',
      ...
      setConfirmed(true);
      setLastResult(json.message || `Transfer Return ${number} confirmed.`);
  ...
  async function handleDelete() {
    if (!retNo) { showToast('This transfer return has not been saved yet'); return; }
    if (!confirm(`Delete transfer return ${retNo}? This cannot be undone.`)) return;
    setDeleting(true);
    try {
      const res = await fetch(
        `/api/inventory/transfer/return/${encodeURIComponent(retNo)}?fromLoc=${encodeURIComponent(fromLoc)}&toLoc=${encodeURIComponent(toLoc)}`,
        { method: 'DELETE' },
      );
      const json = await res.json() as { success?: boolean; message?: string; hint?: string };
      if (!res.ok || !json?.success) {
  ...
  function handleNavigate(key: string, path: string) {
    if (dirty && !confirm('Leave the page without saving the transfer return?')) return;
    router.push(path);
  }
  ...
  async function sendMail() {
    ...
    if (!to) { showToast('Type the address to send the return to', true); return; }
    setSending(true);
    try {
      ...
      if (!res.ok || !json?.success) throw new Error(json?.message || 'The return could not be emailed');
      setMailAsk(false);
      showToast(json.message || 'Emailed ✓');
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'The return could not be emailed', true);
    } finally {
      setSending(false);
    }
  }
  ...
  const locked = confirmed; // a confirmed return is read-only
  ...
  const printData = (() => {
    ...
    <>
      ...
      <div className="po-shell">
        <div className="no-print"><AdminSidebar active="inv-tr-ret" onNav={handleNavigate} onLogout={() => router.push('/admin-login')} /></div>
        ...
        <div className="po-main">
          <header className="po-head no-print">
            ...
            <div className="po-state">
              {retNo ? <span className="chip">{retNo}</span> : <span className="chip dim">not saved yet</span>}
              {retNo && (confirmed ? <span className="chip ok">Confirmed</span> : <span className="chip warn">Pending</span>)}
          ...
          {tab === 'find' && (
            <div className="po-card no-print">
              <div className="po-find">
                ...
                <input value={findQ} onChange={(e) => setFindQ(e.target.value)} placeholder="TR or transfer note number" />
                <label className="rad">
                  <input type="radio" checked={findStatus === 'confirmed'} onChange={() => setFindStatus('confirmed')} /> Confirmed Return Note
                </label>
                <label className="rad">
                  <input type="radio" checked={findStatus === 'pending'} onChange={() => setFindStatus('pending')} /> Pending Return Note
                </label>
                <label className="rad">
              ...
              <div className="po-list-wrap">
                <table className="po-table">
                  <thead>
                    <tr>
                      <th>Return No</th><th>Return Date</th><th>From Location</th><th>To Location</th>
                      <th>Transfer Note</th><th className="num">Net Total</th><th>Status</th><th />
                    </tr>
                  </thead>
                  <tbody>
                    {list.length === 0 && (
                      <tr><td colSpan={8} className="empty">{listBusy ? 'Loading…' : 'No transfer returns matched'}</td></tr>
                    )}
                    {list.map((row) => (
                      <tr key={`${row.fromLocCode}|${row.toLoc}|${row.trtnNo}`}>
                        <td className="mono">{row.trtnNo}</td>
                        <td>{row.trtnDate}</td>
                        <td>{row.fromLocDes || row.fromLocCode}</td>
                        <td>{row.toLocDes || row.toLoc}</td>
                        <td className="mono">{row.tnNo || '—'}</td>
                        ...
                        <td>
                          <button className="btn small" onClick={() => void openReturn(row)}>
            ...
            <div className="po-card">
              <div className="po-form no-print">
                ...
                <select
                  value={tnNo}
                  disabled={locked}
                  onChange={(e) => void loadFromNote(e.target.value)}
                >
                  <option value="">— choose —</option>
                  {tnNo && !tnOptions.some((r) => r.tranNo === tnNo) && (
                    <option value={tnNo}>{tnNo} — the note on this return</option>
                  )}
                  {tnOptions.map((r) => (
                    <option key={`${r.fromLocCode}|${r.toLoc}|${r.tranNo}`} value={r.tranNo}>
                      {r.tranNo} — {r.fromLocDes || r.fromLocCode} → {r.toLocDes || r.toLoc}
                    </option>
                  ))}
                </select>

                <label>Transfer Return No</label>
                <input value={retNo} readOnly placeholder="issued on save" />

                <label>Transfer Note Date</label>
                <input value={tnDate} readOnly placeholder="from the transfer note" />

                <label>Transfer Return Date</label>
                <input type="date" value={retDate} disabled={locked} onChange={(e) => { setRetDate(e.target.value); setDirty(true); }} />

                <label>From Location</label>
                <select
                  value={fromLoc}
                  disabled={locked || !!retNo}
                  title={retNo ? 'Locations are part of the return’s key — Clear to start a new one' : undefined}
              ...
              {tnLoading && <div className="po-note no-print">Loading the transfer note…</div>}
                ...
                <table className="po-table">
                  <thead>
                    <tr>
                      ...
                      <th style={{ width: 100 }} className="num">TN QTY</th>
                      <th style={{ width: 120 }} className="num">Returned QTY</th>
                  ...
                  <tbody>
                    {lines.map((line, i) => (
                      <tr key={line.key}>
                        ...
                        <td>
                          {/* what the note moved — comes from the transfer note,
                              the operator only keys what actually comes back  */}
                          <input
                            type="number" className="num"
                            value={line.tnQty}
                            disabled
                            title="The quantity the transfer note moved"
                          />
                        </td>
                        <td>
                          <input
                            type="number" min="0" step="0.001" className="num"
                            value={line.retQty} disabled={locked}
                            onChange={(e) => patchLine(line.key, { retQty: e.target.value })}
                          />
                        </td>
                        <td className="num">{money(lineValue(line.costPrice, line.retQty))}</td>
      ...
      {printAsk && (
        <div className="ask-back no-print" role="dialog" aria-modal="true" aria-label="Print transfer return">
          <div className="ask-card">
            <h2>Print Transfer Return Note</h2>
            <p>
              {retNo ? <><span className="mono">{retNo}</span> — </> : null}
      ...
      {mailAsk && (
        <div className="ask-back no-print" role="dialog" aria-modal="true" aria-label="Email transfer return">
          <div className="ask-card mail-card">
            <h2>Email Transfer Return Note</h2>
            <p>
              <span className="mono">{retNo}</span> —{' '}
            ...
            <div className="mail-attach">
              Attachment: <b>{(retNo || 'transfer-return').trim()}.pdf</b> ·{' '}
