...
const newLine = (): TrLine => ({
  key: `R${++lineSeq}`, itemCode: '', name: '', unitID: '', costPrice: '', trQty: '',
});
...
export default function TransferRequisitionPage() {
  ...
  const [trDueDate, setTrDueDate] = useState(dayOf(new Date().toISOString()));
  ...
  const unitName = useCallback(
    ...
    try {
      ...
      const json = await res.json() as {
        ...
        data?: {
          header: { trNo: string; fromLocCode: string; toLoc: string; trDate: string; trDueDate: string; remarks: string; confirmed: boolean };
      ...
      setToLoc(h.toLoc);
      setTrDate(dayOf(h.trDate));
      setTrDueDate(dayOf(h.trDueDate));
      ...
      setLastResult('');
      setTab('details');
      showToast(`Transfer requisition ${h.trNo} loaded`);
    } catch (err) {
  ...
  async function handleSave(): Promise<boolean> {
    ...
    try {
      const res = reqNo
        ? await fetch(`/api/inventory/transfer/requisition/${encodeURIComponent(reqNo)}?fromLoc=${encodeURIComponent(fromLoc)}&toLoc=${encodeURIComponent(toLoc)}`, {
            method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
          })
        : await fetch('/api/inventory/transfer/requisition', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...body, confirm: false }),
          });
      const json = await res.json() as {
        success?: boolean; message?: string; hint?: string; data?: { trNo: string; netTotal: number };
      };
      if (!res.ok || !json?.success) {
        throw new Error(json?.hint ? `${json.message} — ${json.hint}` : (json?.message || 'Save failed'));
      }
      if (json.data?.trNo) setReqNo(json.data.trNo);
      setDirty(false);
    ...
    try {
      ...
      if (!confirm(`Confirm transfer requisition ${number}?\nA confirmed requisition can no longer be edited or deleted.`)) return;

      const res = await fetch(`/api/inventory/transfer/requisition/${encodeURIComponent(number)}/confirm`, {
        method: 'POST',
      ...
      setConfirmed(true);
      setLastResult(json.message || `Transfer requisition ${number} confirmed.`);
  ...
  async function handleDelete() {
    ...
    if (!confirm(`Delete transfer requisition ${reqNo}? This cannot be undone.`)) return;
    setDeleting(true);
    try {
      const res = await fetch(
        `/api/inventory/transfer/requisition/${encodeURIComponent(reqNo)}?fromLoc=${encodeURIComponent(fromLoc)}&toLoc=${encodeURIComponent(toLoc)}`,
        { method: 'DELETE' },
      );
      const json = await res.json() as { success?: boolean; message?: string; hint?: string };
      if (!res.ok || !json?.success) {
  ...
  async function sendMail() {
    ...
    if (!to) { showToast('Type the address to send the requisition to', true); return; }
    setSending(true);
    try {
      ...
      if (!res.ok || !json?.success) throw new Error(json?.message || 'The requisition could not be emailed');
      setMailAsk(false);
      showToast(json.message || 'Emailed ✓');
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'The requisition could not be emailed', true);
    } finally {
      setSending(false);
    }
  }
  ...
  const printData = (() => {
    ...
    <>
      ...
      <div className="po-shell">
        <div className="no-print"><AdminSidebar active="inv-tr-req" onNav={handleNavigate} onLogout={() => router.push('/admin-login')} /></div>
            ...
            <div className="po-card no-print">
              ...
              <div className="po-list-wrap">
                <table className="po-table">
                  <thead>
                    <tr>
                      <th>TR No</th><th>TR Date</th><th>From Location</th><th>To Location</th>
                  ...
                  <tbody>
                    ...
                    {list.map((row) => (
                      <tr key={`${row.fromLocCode}|${row.toLoc}|${row.trNo}`}>
                        <td className="mono">{row.trNo}</td>
                        <td>{row.trDate}</td>
          ...
          {tab === 'details' && (
            <div className="po-card">
              <div className="po-form no-print">
                ...
                <input type="date" value={trDueDate} disabled={locked} onChange={(e) => { setTrDueDate(e.target.value); setDirty(true); }} />
                  ...
                  <tbody>
                    {lines.map((line, i) => (
                      <tr key={line.key}>
                        ...
                        <td className="num">{money(lineValue(line.costPrice, line.trQty))}</td>
      ...
      {printAsk && (
        <div className="ask-back no-print" role="dialog" aria-modal="true" aria-label="Print transfer requisition">
          <div className="ask-card">
            <h2>Print Transfer Requisition Note</h2>
            <p>
      ...
      {mailAsk && (
        <div className="ask-back no-print" role="dialog" aria-modal="true" aria-label="Email transfer requisition">
          <div className="ask-card mail-card">
            <h2>Email Transfer Requisition Note</h2>
            <p>
            ...
            <div className="mail-attach">
              Attachment: <b>{(reqNo || 'transfer-requisition').trim()}.pdf</b> ·{' '}
