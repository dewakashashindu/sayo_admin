...
// TRANSFER REQUISITION NOTE — UI only (galapena PO wage)
// Legacy: From Location / To Location / TR Due Date / Transfer Req No (TC) / TR Date
//         Item Code | Item Name | Unit | Cost Price | TR QTY | Item Value
//         Remarks | Net Value | Buttons: Clear · Confirmation · Print · Delete · Save · Cancel
//         Find: TRANSFER REQUISITION DETAIL — FromLocCode/LocDes/ToLoc/ToLocDes/TRNO/TRDate
...
let seq = 0;
const newLine = (): TrLine => ({ key: `R${++seq}`, itemCode:'', itemName:'', unitID:'', costPrice:'', trQty:'', itemValue:0 });
...
export default function TransferRequisitionPage(){
  ...
  const [fromLoc,setFromLoc]=useState('KITCHEN');
  const [toLoc,setToLoc]=useState('MILLA MIRISSA');
  const [trDueDate,setTrDueDate]=useState('2026-09-25');
  const [trDate,setTrDate]=useState('2026-09-18');
  const [reqNo,setReqNo]=useState('TC000006');
  ...
  useEffect(()=>{ let a=true; (async()=>{ try{ const r=await fetch('/api/inventory/lookups',{cache:'no-store'}); const j=await r.json() as any; if(!a) return; if(r.ok&&j?.success){ const locs=j.locations??[]; if(locs.length){ setLocations(locs); setUnits(j.units??[]); if(j.company?.name) setCompany(j.company); setLookupNote(`${locs.length} location(s) loaded`); } else { throw new Error('empty'); } } else throw new Error('fail'); } catch{ if(!a) return; setLocations([{code:'KITCHEN',des:'KITCHEN',address:'',enable:true},{code:'01',des:'MILLA MIRISSA',address:'',enable:true},{code:'03',des:'KITCHEN',address:'',enable:true},{code:'D1',des:'MILLA MIRISSA',address:'',enable:true}]); setUnits([{id:'KILOGRAM',des:'KILOGRAM',enable:true},{id:'KILOGRA',des:'KILOGRAM',enable:true},{id:'PCS',des:'PCS',enable:true}]); setLookupNote('Demo locations — API not reachable'); } })(); return()=>{a=false}; },[]);

  const unitName = (id:string)=> units.find(u=>u.id===id)?.des || id || '—';
  const netValue = useMemo(()=> lines.reduce((s,l)=> s+ (Number(l.costPrice)||0)*(Number(l.trQty)||0),0),[lines]);

  function patchLine(key:string,p:Partial<TrLine>){
    setLines(prev=> prev.map(l=>{
      if(l.key!==key) return l;
      const n={...l,...p} as TrLine;
      n.itemValue=(Number(n.costPrice)||0)*(Number(n.trQty)||0);
      return n;
    })); setDirty(true);
  }
  function pickItem(key:string,item:SuggestedItem){
    patchLine(key,{itemCode:item.code,itemName:item.des,unitID:item.masterUnitID||'',costPrice:item.costPrice? String(item.costPrice):''});
  }
  ...
  async function handleSave(confirm=false){
    ...
    if(valid.some(l=> !(Number(l.trQty)>0))){ show('Enter TR QTY for each line', true); return; }
    try{
      const body = { fromLocCode: fromLoc, toLoc: toLoc, trDate, trDueDate, remarks, confirm, lines: valid.map(l=>({ itemCode:l.itemCode||l.itemName, unitID:l.unitID, costPrice: Number(l.costPrice)||0, trQty: Number(l.trQty)||0 })) };
      const res = await fetch('/api/inventory/transfer/requisition',{method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
      const j = await res.json() as any;
      if(!res.ok || !j?.success) throw new Error(j?.message||'Save failed');
      setReqNo(j.data.trNo); setConfirmed(j.data.confirmed); setDirty(false); show(j.message||'Saved ✓');
      // update find list
      void loadList();
    }catch(e:any){ show(e?.message||'Save failed', true); }
  }
  function handleClear(){ setLines([newLine(),newLine(),newLine(),newLine()]); setRemarks(''); setDirty(false); show('Cleared'); }
  async function openReq(row: FindRow){
    try{
      const res = await fetch(`/api/inventory/transfer/requisition/${encodeURIComponent(row.trNo)}?fromLoc=${encodeURIComponent(row.fromCode)}&toLoc=${encodeURIComponent(row.toCode)}`,{cache:'no-store'});
      const j = await res.json() as any;
      if(!res.ok || !j?.success) throw new Error(j?.message||'load failed');
      const h=j.data.header;
      setFromLoc(h.fromLocCode); setToLoc(h.toLoc); setReqNo(h.trNo); setTrDate(String(h.trDate).slice(0,10)); setTrDueDate(String(h.trDueDate).slice(0,10)); setRemarks(h.remarks||''); setConfirmed(h.confirmed);
      setLines((j.data.lines||[]).map((r:any)=>({key:`R${++seq}`, itemCode:r.itemCode, itemName:r.itemName, unitID:r.unitID, costPrice:String(r.costPrice), trQty:String(r.trQty), itemValue:Number(r.itemValue||0)})));
      setDirty(false); setTab('details'); show(`Requisition ${row.trNo} loaded`);
    }catch(e:any){ show(e?.message||'Could not load requisition', true); }
  }
  ...
  function handlePrint(){
    ...
    if(!reqNo || reqNo==='' ){ show('Save the requisition before printing', true); return; }
    setPrintAsk(true);
  }
  ...
  const printData = (()=> {
    ...
    return {
      ...
      dueDate: poPrintDate(trDueDate),
      printDate: poPrintClock(at).date,
      printTime: poPrintClock(at).time,
      user: 'aura',
      rows: poPrintRows(printableLines.map(l=>({itemCode:l.itemCode, name:l.itemName, unitID:l.unitID, unitName: units.find(u=>u.id===l.unitID)?.des||l.unitID, costPrice:Number(l.costPrice)||0, poQty:Number(l.trQty)||0}))),
      total: poPrintTotal(printableLines.map(l=>({itemCode:l.itemCode,name:l.itemName,unitID:l.unitID,costPrice:Number(l.costPrice)||0, poQty:Number(l.trQty)||0}))),
  ...
  function openMailDialog(){
    ...
    if(!reqNo){ show('Save the requisition first (Save), then it can be emailed', true); return; }
    setMailTo(prev=> prev || '');
    setMailSubject(prev=> prev || `Transfer Requisition Note ${reqNo}`);
  ...
  async function sendMail(){
    ...
    setSending(true);
    try{
      const res=await fetch(`/api/inventory/transfer/requisition/${encodeURIComponent(reqNo)}/email`,{
  ...
  const loadList = async()=>{
    ...
    try{
      ...
      const res = await fetch(`/api/inventory/transfer/requisition?${params.toString()}`,{cache:'no-store'});
      const j = await res.json() as any;
      if(res.ok && j?.success && Array.isArray(j.data)){
        setFindRows(j.data.map((r:any)=>({fromCode:r.fromLocCode, fromDes:r.fromLocDes||r.fromLocCode, toCode:r.toLoc, toDes:r.toLocDes||r.toLoc, trNo:r.trNo, trDate:String(r.trDate??'').slice(0,10)})));
        if(j.data.length===0) show('No requisitions matched — save one from Details');
      } else throw new Error(j?.message||'load failed');
    }catch(e:any){
      show(e?.message||'Could not load requisitions', true);
      setFindRows([]);
  ...
  return (
    <>
      ...
      <div className="po-shell">
        <div className="no-print"><AdminSidebar active="inv-tr-req" onNav={handleNav} onLogout={()=>router.push('/admin-login')} /></div>
        <div className="po-main">
          <header className="po-head no-print">
            <h1>TRANSFER REQUISITION NOTE</h1>
            ...
            <div className="po-state">
              <span className="chip">{reqNo}</span>
          ...
          {lookupNote && <div className="po-note no-print">{lookupNote} — From/To locations like legacy TC screen</div>}

          {tab==='find' && (
            <div className="po-card no-print">
              <div className="po-list-wrap">
                <table className="po-table">
                  <thead><tr><th>FromLocCode</th><th>LocDes</th><th>ToLoc</th><th>ToLocDes</th><th>TRNO</th><th>TRDate</th></tr></thead>
                  <tbody>
                    {filteredFind.length===0 && <tr><td colSpan={6} className="empty">{listBusy?'Loading…':'No requisitions matched'}</td></tr>}
                    {filteredFind.map(r=>(
                      <tr key={r.trNo} style={{background:'#eaffea',cursor:'pointer'}} onClick={()=>void openReq(r)}>
                        <td className="mono" style={{color:'#0b5cab',background:'#eaffea'}}>{r.fromCode}</td>
                        <td style={{color:'#0b5cab',background:'#eaffea'}}>{r.fromDes}</td>
                        <td style={{color:'#0b5cab',background:'#eaffea'}}>{r.toCode}</td>
                        <td style={{color:'#0b5cab',background:'#eaffea'}}>{r.toDes}</td>
                        <td className="mono" style={{color:'#0b5cab',background:'#eaffea',textDecoration:'underline'}}>{r.trNo}</td>
                        <td style={{color:'#0b5cab',background:'#eaffea'}}>{dayOf(r.trDate)}</td>
                      </tr>
                    ))}
                    <tr style={{height:40}}><td colSpan={6}></td></tr>
                  </tbody>
                </table>
              </div>
              <div className="po-find">
                <label>Find Criteria</label>
                <input value={findQ} onChange={e=>setFindQ(e.target.value)} placeholder="TC number or location" />
                <label className="rad"><input type="radio" checked={findStatus==='confirmed'} onChange={()=>setFindStatus('confirmed')} /> Confirmed IRN</label>
                <label className="rad"><input type="radio" checked={findStatus==='pending'} onChange={()=>setFindStatus('pending')} /> Pending IRN</label>
                <button className="btn" onClick={()=>{setListBusy(true); setTimeout(()=>setListBusy(false),500);}}>Find</button>
              </div>
              <div style={{display:'flex',gap:8}}>
                <button className="btn" style={{flex:1}}>Find</button>
                <button className="btn" style={{flex:1,background:'#eef4f4'}}>Details</button>
              </div>
              <div className="po-note" style={{background:'#fff',border:'1px solid #000',padding:10}}>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:'11px'}}>
                  <div><b>MILLA MIRISSA</b><div style={{color:'#555'}}>ADD ADD</div></div>
                  <div style={{textAlign:'center',fontWeight:800}}>Transfer Requisition Note</div>
                  <div style={{fontSize:'10px',textAlign:'right'}}>Print Date : 21-Sep-2026<br/>Print Time : 9:51:50 am<br/>User : aura</div>
                </div>
                <div style={{display:'flex',gap:40,fontSize:'11px',marginTop:10}}><span>From <b>{fromLoc || '—'}</b> Req Date {trDate}</span><span>To <b>{toLoc || '—'}</b> Due Date {trDueDate}</span><span>Issue Req No <b>{reqNo}</b> Txn Date {trDate}</span></div>
                <table className="po-table" style={{marginTop:8}}>
                  <thead><tr><th>Item Code</th><th>Row Item Description</th><th>Unit Des</th><th className="num">TR Qty</th><th className="num">Cost Price</th><th className="num">Item Cost</th></tr></thead>
                  <tbody>
                    <tr><td colSpan={6} style={{background:'#f7eacc',fontWeight:700}}>{fromLoc} → {toLoc} {remarks && `— ${remarks}`}</td></tr>
                    {lines.length===0 ? <tr><td colSpan={6} className="empty">No lines — select a requisition from above or add in Details</td></tr> : lines.filter(l=>l.itemCode||l.itemName).map(l=>(
                      <tr key={l.key}><td>{l.itemCode || '—'}</td><td>{l.itemName || '—'}</td><td>{unitName(l.unitID)}</td><td className="num">{l.trQty || '—'}</td><td className="num">{l.costPrice ? money(Number(l.costPrice)) : '—'}</td><td className="num">{money(l.itemValue)}</td></tr>
                    ))}
                    <tr><td colSpan={5} style={{textAlign:'right',fontSize:'10px'}}>{remarks || '—'}</td><td className="num" style={{background:'#baddf7',fontWeight:800}}>{money(netValue)}</td></tr>
                  </tbody>
                </table>
              </div>
            </div>
          ...
          {tab==='details' && (
            <div className="po-card">
              <div className="po-form no-print" style={{gridTemplateColumns:'110px minmax(160px,1fr) 100px minmax(140px,1fr) 120px minmax(150px,1fr)'}}>
                ...
                <input type="date" value={trDueDate} onChange={e=>{setTrDueDate(e.target.value); setDirty(true);}} />
                <label>Transferr Req No</label>
                <input value={reqNo} readOnly className="mono" style={{background:'#fff8dc',fontWeight:700}} />
                ...
                <table className="po-table">
                  <thead>
                    <tr>
                      ...
                      <th className="no-print" style={{width:38}}></th>
                    </tr>
                  </thead>
                  <tbody>
                    {lines.map((l,i)=>(
                      <tr key={l.key} style={i===0?{background:'#5865ff'}:{}}>
                        <td className="num" style={i===0?{background:'#5865ff',color:'#fff'}:{}}>{i===lines.length-1 && !l.itemCode ? '*' : '▶'}</td>
                        <td style={i===0?{background:'#2244ff',color:'#fff'}:{}}>{l.itemCode || <ItemSuggestInput locCode={fromLoc} value={l.itemName} onText={t=>patchLine(l.key,{itemName:t,itemCode:''})} onPick={it=>pickItem(l.key,it)} />}</td>
                        <td style={i===0?{background:'#2244ff',color:'#fff'}:{}}>{i===0 ? l.itemName : <ItemSuggestInput locCode={fromLoc} value={l.itemName} onText={t=>patchLine(l.key,{itemName:t})} onPick={it=>pickItem(l.key,it)} />}{l.itemCode && i!==0 && <div className="code-hint">{l.itemCode}</div>}</td>
                        <td>
                          <select value={l.unitID} onChange={e=>patchLine(l.key,{unitID:e.target.value})} style={i===0?{background:'#2244ff',color:'#fff',borderColor:'#2244ff'}:{}}>
                            <option value="">—</option>
                            {units.map(u=><option key={u.id} value={u.id}>{u.des}</option>)}
                            {!units.find(u=>u.id===l.unitID) && l.unitID && <option value={l.unitID}>{l.unitID}</option>}
                          </select>
                        </td>
                        <td className="num"><input type="number" className="num" value={l.costPrice} onChange={e=>patchLine(l.key,{costPrice:e.target.value})} style={i===0?{background:'#2244ff',color:'#fff',borderColor:'#2244ff',textAlign:'right'}:{}} /></td>
                        <td className="num" style={i===0?{background:'#2244ff'}:{background:'#fffde6'}}><input type="number" className="num" value={l.trQty} onChange={e=>patchLine(l.key,{trQty:e.target.value})} style={i===0?{background:'#2244ff',color:'#fff',borderColor:'#2244ff'}:{background:'#fffde6'}} /></td>
              ...
              <div className="po-form no-print" style={{gridTemplateColumns:'80px 1fr 90px 140px'}}>
                <label>Remarks</label>
                <textarea value={remarks} onChange={e=>{setRemarks(e.target.value); setDirty(true);}} rows={2} style={{gridColumn:'span 1',border:'1px solid rgba(30,58,64,0.18)',borderRadius:7,padding:'8px 9px',fontFamily:'inherit',fontSize:'12.5px'}} />
              ...
              <div className="po-actions no-print">
                ...
                <button className="btn danger" onClick={()=>show('Delete — use API to delete if needed')}>Delete</button>
                <button className="btn primary" onClick={()=>handleSave(false)} disabled={locked}>Save</button>
      ...
      {printAsk && (
        <div className="ask-back" onClick={()=>setPrintAsk(false)}>
          <div className="ask-card" onClick={e=>e.stopPropagation()}>
            ...
            <p>Standard shows cost, Supplier hides it — same as Purchase Orders.</p>
      ...
      {printJob && (
        <TransferPrintSheet
          title="Transfer Requisition Note"
      ...
      {mailAsk && (
        <div className="ask-back" onClick={()=>setMailAsk(false)}>
          <div className="ask-card mail-card" onClick={e=>e.stopPropagation()}>
            <h2>Email Transfer Requisition {reqNo}</h2>
            ...
            <div className="mail-attach">Attachment: {reqNo||'transfer'}.pdf — {printData.rows.length} line(s)</div>
