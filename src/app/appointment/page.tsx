...
const TIME_SLOTS   = ['8:00 AM','8:30 AM','9:00 AM','9:30 AM','10:00 AM','10:30 AM','11:00 AM','11:30 AM','12:00 PM','12:30 PM','1:00 PM','1:30 PM','2:00 PM','2:30 PM','3:00 PM','3:30 PM','4:00 PM','4:30 PM','5:00 PM','5:30 PM','6:00 PM','6:30 PM','7:00 PM','7:30 PM','8:00 PM','8:30 PM','9:00 PM','9:30 PM','10:00 PM','10:30 PM','11:00 PM','11:30 PM'];
const SLOT_MINUTES       = 30;
const GRID_START_MINUTES = parseSlotToMinutes('8:00 AM');
const ROW_HEIGHT_PX      = 52;

/* ─────────────────────────────────────────
   CSS  (unchanged — paste your full CSS block here)
───────────────────────────────────────── */
const CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  @keyframes timelinePulse { 0%{opacity:1;} 50%{opacity:0.6;} 100%{opacity:1;} }
  ...
  .now-line::before {
    content:''; position:absolute; left:76px; top:50%; transform:translateY(-50%);
  ...
  .now-label {
    ...
    background:rgba(255,255,255,0.9); padding:1px 4px; border-radius:4px; line-height:1;
  }
  ...
  .stats-row-wrap { display:flex; gap:10px; align-items:stretch; flex-wrap:wrap; }
  .stat-card { flex:1 1 0; min-width:80px; border-radius:12px; padding:14px 16px; display:flex; flex-direction:column; gap:4px; box-shadow:0 2px 8px rgba(0,0,0,0.05); position:relative; overflow:hidden; }
  .stat-card .sc-label { font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:0.07em; opacity:0.7; display:flex; align-items:center; gap:5px; }
  .stat-card .sc-value { font-size:30px; font-weight:800; line-height:1.1; color:#1f2937; }
  .stat-card .sc-sub   { font-size:10px; font-weight:500; opacity:0.55; margin-top:2px; }
  .stat-card .sc-icon  { position:absolute; right:12px; top:50%; transform:translateY(-50%); opacity:0.12; }
  .stat-card .sc-bar   { position:absolute; bottom:0; left:0; height:3px; border-radius:0 0 12px 12px; }

  .period-tab { padding:6px 14px; border-radius:7px; border:none; font-family:'Inter',sans-serif; font-size:12px; font-weight:500; cursor:pointer; white-space:nowrap; transition:all 0.15s; background:transparent; color:#4b5563; }
  .period-tab.active { background:#1e3a40; color:#fff; font-weight:700; box-shadow:0 1px 4px rgba(30,58,64,0.35); }
...
const Ico = {
  ...
  Appt:     ({s=32}:{s?:number})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/><line x1="8" y1="15" x2="16" y2="15"/></svg>,
  Check:    ({s=32}:{s?:number})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>,
  Pending:  ({s=32}:{s?:number})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
  Cancel:   ({s=32}:{s?:number})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>,
  Live:     ({s=32}:{s?:number})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="4"/><path d="M4.93 4.93a10 10 0 0 0 0 14.14M19.07 4.93a10 10 0 0 1 0 14.14"/></svg>,
...
function NotificationPanel({ appts }: { appts: Appointment[] }) {
  const recent = useMemo(()=>{
    ...
    return [...appts]
      ...
      .filter(a=>{if(seen.has(a.bookingID))return false;seen.add(a.bookingID);return true;})
      .slice(0,8);
  },[appts]);
  function dotColor(s:string){
    if(s==='cancelled')return'#ef4444';if(s==='confirmed')return'#22c55e';
    if(s==='ongoing')return'#3b82f6';return'#f59e0b';
  }
  return (
    <div className="notif-dropdown pop-in">
      <div style={{padding:'12px 14px',borderBottom:'1px solid #eee',fontWeight:700,fontSize:13,color:'#1e3a40',fontFamily:"'Inter',sans-serif"}}>Notifications</div>
      <div style={{maxHeight:340,overflowY:'auto'}}>
        {recent.length===0
          ?<div style={{padding:24,textAlign:'center',color:'#9ca3af',fontSize:12,fontFamily:"'Inter',sans-serif"}}>No recent activity</div>
          :recent.map(a=>(
            <div key={a.bookingID} className="notif-item">
              ...
              <div style={{flex:1,minWidth:0}}>
                <p style={{fontSize:12,fontWeight:600,color:'#1f2937',fontFamily:"'Inter',sans-serif",overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{a.clientName} · {a.serviceName.split(',')[0]}</p>
                <p style={{fontSize:11,color:'#9ca3af',fontFamily:"'Inter',sans-serif",marginTop:1}}>{fmtRelTime(a.txnDateTime)} · {a.status}</p>
              </div>
            </div>
          ))}
      </div>
...
function StatsRow({ stats, onNewAppt }: { stats: Stats; onNewAppt: ()=>void }) {
  ...
  return (
    <div className="stats-row-wrap">
      <div className="stat-card" style={{background:'linear-gradient(135deg,#1e3a40,#2a5260)',border:'none',minWidth:160,flex:'1.4 1 160px'}}>
        <div className="sc-label" style={{color:'rgba(255,255,255,0.6)'}}><Ico.Appt s={12}/> Total Appointments</div>
        <div className="sc-value" style={{color:'#fff',fontSize:38}}>{stats.total}</div>
        <div className="sc-sub" style={{color:'rgba(255,255,255,0.45)'}}>{fmtDateNav(todayISO())}</div>
        <div className="sc-icon" style={{opacity:0.08}}><Ico.Appt s={64}/></div>
        <div className="sc-bar" style={{background:'rgba(255,255,255,0.18)',width:'100%'}}/>
      </div>
      <div className="stat-card" style={{background:'linear-gradient(135deg,#f0fdf4,#dcfce7)',border:'1px solid rgba(34,197,94,0.25)',flex:'1 1 110px'}}>
        <div className="sc-label" style={{color:'#15803d'}}><span style={{width:7,height:7,borderRadius:'50%',background:'#22c55e',display:'inline-block'}}/>Confirmed</div>
        <div className="sc-value" style={{color:'#14532d'}}>{stats.confirmed}</div>
        <div className="sc-sub" style={{color:'#16a34a'}}>{pct(stats.confirmed)}% of total</div>
        <div className="sc-icon"><Ico.Check s={40}/></div>
        <div className="sc-bar" style={{background:'#22c55e',width:`${pct(stats.confirmed)}%`}}/>
      </div>
      <div className="stat-card" style={{background:'linear-gradient(135deg,#eff6ff,#dbeafe)',border:'1px solid rgba(59,130,246,0.3)',flex:'1 1 110px',animation:stats.ongoing>0?'livePulseBlue 2s infinite':undefined}}>
        <div className="sc-label" style={{color:'#1d4ed8'}}><span className="live-dot-blue"/>Ongoing</div>
        <div className="sc-value" style={{color:'#1e3a8a'}}>{stats.ongoing}</div>
        <div className="sc-sub" style={{color:'#2563eb'}}>{stats.ongoing>0?'In session now':'None active'}</div>
        <div className="sc-icon"><Ico.Live s={40}/></div>
        <div className="sc-bar" style={{background:'#3b82f6',width:`${pct(stats.ongoing)}%`}}/>
      </div>
      <div className="stat-card" style={{background:'linear-gradient(135deg,#fffbeb,#fef3c7)',border:'1px solid rgba(245,158,11,0.3)',flex:'1 1 110px'}}>
        <div className="sc-label" style={{color:'#b45309'}}><span style={{width:7,height:7,borderRadius:'50%',background:'#f59e0b',display:'inline-block'}}/>Pending</div>
        <div className="sc-value" style={{color:'#78350f'}}>{stats.pending}</div>
        <div className="sc-sub" style={{color:'#d97706'}}>{pct(stats.pending)}% of total</div>
        <div className="sc-icon"><Ico.Pending s={40}/></div>
        <div className="sc-bar" style={{background:'#f59e0b',width:`${pct(stats.pending)}%`}}/>
      </div>
      <div className="stat-card" style={{background:'linear-gradient(135deg,#fff1f2,#fee2e2)',border:'1px solid rgba(239,68,68,0.25)',flex:'1 1 110px'}}>
        <div className="sc-label" style={{color:'#b91c1c'}}><span style={{width:7,height:7,borderRadius:'50%',background:'#ef4444',display:'inline-block'}}/>Cancelled</div>
        <div className="sc-value" style={{color:'#7f1d1d'}}>{stats.cancelled}</div>
        <div className="sc-sub" style={{color:'#dc2626'}}>{pct(stats.cancelled)}% of total</div>
        <div className="sc-icon"><Ico.Cancel s={40}/></div>
        <div className="sc-bar" style={{background:'#ef4444',width:`${pct(stats.cancelled)}%`}}/>
      </div>
      <button onClick={onNewAppt}
        style={{minWidth:130,minHeight:90,background:'linear-gradient(135deg,#1e3a40,#2a5260)',border:'none',borderRadius:12,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:6,cursor:'pointer',fontFamily:"'Inter',sans-serif",boxShadow:'0 2px 10px rgba(30,58,64,0.3)',transition:'all 0.18s',flexShrink:0}}
        onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow='0 6px 20px rgba(30,58,64,0.4)';}}
...
function CalendarPopup({ value, onChange, onClose }: { value:string; onChange:(iso:string)=>void; onClose:()=>void }) {
  ...
  for(let i=0;i<firstDow;i++){const pm=calMonth===0?11:calMonth-1,py=calMonth===0?calYear-1:calYear;cells.push({d:prevDays-firstDow+1+i,m:pm,y:py,cur:false});}
  for(let i=1;i<=daysInMonth;i++) cells.push({d:i,m:calMonth,y:calYear,cur:true});
  for(let i=1;i<=42-cells.length;i++){const nm=calMonth===11?0:calMonth+1,ny=calMonth===11?calYear+1:calYear;cells.push({d:i,m:nm,y:ny,cur:false});}
  return (
    <div className="cal-popup pop-in">
      <div className="cal-header">
        <button className="cal-nav" onClick={()=>{if(view==='day'){calMonth===0?(setCalMonth(11),setCalYear(y=>y-1)):setCalMonth(m=>m-1);}if(view==='month'){setCalYear(y=>y-1);}if(view==='year'){setYearBase(b=>b-16);}}}><Ico.ChevL s={14}/></button>
        <button className="cal-month-title" onClick={()=>{if(view==='day')setView('month');else if(view==='month')setView('year');else setView('day');}}>
          {view==='day'&&`${MONTHS[calMonth]} ${calYear}`}{view==='month'&&calYear}{view==='year'&&`${yearBase}–${yearBase+15}`}
        </button>
        <button className="cal-nav" onClick={()=>{if(view==='day'){calMonth===11?(setCalMonth(0),setCalYear(y=>y+1)):setCalMonth(m=>m+1);}if(view==='month'){setCalYear(y=>y+1);}if(view==='year'){setYearBase(b=>b+16);}}}><Ico.ChevR s={14}/></button>
      </div>
      {view==='day'&&(<><div className="cal-grid">{['Su','Mo','Tu','We','Th','Fr','Sa'].map(d=><div key={d} className="cal-daylbl">{d}</div>)}</div><div className="cal-grid">{cells.map((c,i)=>{const isToday=c.d===todayD.getDate()&&c.m===todayD.getMonth()&&c.y===todayD.getFullYear();const isSel=c.d===selDay&&c.m===selMonth&&c.y===selYear;return(<button key={i} className={`cal-day${!c.cur?' other-month':''}${isSel?' selected':''}${isToday&&!isSel?' today':''}`} onClick={()=>{onChange(isoFromYMD(c.y,c.m,c.d));onClose();}}>{c.d}</button>);})}</div></>)}
      {view==='month'&&(<div className="month-grid">{MONTHS_SHORT.map((mo,i)=><button key={mo} className={`month-item${i===selMonth&&calYear===selYear?' sel-mo':''}`} onClick={()=>{setCalMonth(i);setView('day');}}>{mo}</button>)}</div>)}
      {view==='year'&&(<div className="year-grid">{Array.from({length:16},(_,i)=>yearBase+i).map(yr=><button key={yr} className={`year-item${yr===selYear?' sel-yr':''}`} onClick={()=>{setCalYear(yr);setView('month');}}>{yr}</button>)}</div>)}
      <div style={{textAlign:'center',marginTop:10}}>
        <button onClick={()=>{onChange(todayISO());onClose();}} style={{fontSize:11,fontWeight:600,color:'#1e3a40',background:'rgba(30,58,64,0.07)',border:'none',borderRadius:6,padding:'4px 14px',cursor:'pointer',fontFamily:"'Inter',sans-serif"}}>Today</button>
      </div>
    </div>
  );
}
...
function ModalHeader({ title, sub, onClose }: { title:string; sub:string; onClose:()=>void }) {
  return (
    <>
      <div style={{display:'flex',justifyContent:'center',padding:'12px 0 0'}}><div style={{width:40,height:4,borderRadius:99,background:'#e5e7eb'}}/></div>
      <div style={{background:'#1e3a40',padding:'16px 20px 18px',display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:10}}>
        ...
        <button onClick={onClose} style={{background:'rgba(255,255,255,0.12)',border:'none',borderRadius:'50%',width:34,height:34,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'#fff'}}><Ico.X/></button>
      </div>
    </>
  );
}
      ...
      <div className="modal-box slide-up" onClick={e=>e.stopPropagation()} style={{maxWidth:540}}>
        ...
        <div style={{padding:'18px 20px 28px',display:'flex',flexDirection:'column',gap:14}}>
          <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
            <StatusBadge status={appt.status}/><ModeBadge mode={appt.mode}/>
            ...
            {appt.guests.length>1&&<span className="badge b-wlk"><Ico.Users/>{appt.guests.length} guests</span>}
          </div>
          {appt.serviceNames.length>1&&(
            <div style={{background:'#f0f8f9',border:'1px solid #c8dce0',borderRadius:10,padding:'10px 14px'}}>
              ...
              {appt.serviceNames.map((s,i)=><p key={i} style={{fontSize:13,color:'#1e3a40',fontWeight:600,marginBottom:2}}>• {s}</p>)}
            </div>
          )}
          ...
          {appt.status==='pending'&&(
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              <div className="modal-action-row">
                <button className="btn-modal-cancel"  onClick={()=>{onStatusChange(appt.bookingID,appt.locCode,'cancelled');onClose();}}><Ico.XCircle s={18}/> Cancel</button>
                <button className="btn-modal-confirm" onClick={()=>{onStatusChange(appt.bookingID,appt.locCode,'confirmed');onClose();}}><Ico.CheckCircle s={18}/> Confirm</button>
              </div>
              <button className="btn-modal-reschedule" onClick={()=>{onReschedule(appt);onClose();}}><Ico.Reschedule s={18}/> Reschedule</button>
            </div>
          )}
          {appt.status==='confirmed'&&(
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              <div className="modal-action-row">
                <button className="btn-modal-cancel"  onClick={()=>{onStatusChange(appt.bookingID,appt.locCode,'cancelled');onClose();}}><Ico.XCircle s={18}/> Cancel</button>
                <button className="btn-modal-checkin" onClick={()=>{onStatusChange(appt.bookingID,appt.locCode,'ongoing');onClose();}}><Ico.Login s={18}/> Check In</button>
              </div>
              <button className="btn-modal-reschedule" onClick={()=>{onReschedule(appt);onClose();}}><Ico.Reschedule s={18}/> Reschedule</button>
            </div>
          )}
          {appt.status==='ongoing'&&(
            <button className="btn-modal-bill" onClick={()=>{onGoToBill(appt);onClose();}}><Ico.Receipt s={18}/> Go to Bill</button>
          )}
          {appt.status==='cancelled'&&(
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              <div style={{background:'#fef2f2',border:'1px solid #fca5a5',borderRadius:10,padding:'12px 14px',fontSize:13,color:'#991b1b',textAlign:'center',fontWeight:600}}>This booking has been cancelled.</div>
              <button className="btn-modal-reschedule" onClick={()=>{onReschedule(appt);onClose();}}><Ico.Reschedule s={18}/> Reschedule</button>
            </div>
          )}
        </div>
...
function NowLine({ isToday }: { isToday: boolean }) {
  ...
  useEffect(()=>{
    const id=setInterval(()=>setMins(nowMinutes()),30000);
    return()=>clearInterval(id);
  },[]);
  if (!isToday) return null;
  const theadH = 41;
  const minsFromStart = mins - GRID_START_MINUTES;
  const totalMins = TIME_SLOTS.length * SLOT_MINUTES;
  if (minsFromStart < 0 || minsFromStart > totalMins) return null;
  const topPx = theadH + (minsFromStart / SLOT_MINUTES) * ROW_HEIGHT_PX;
  return (
    <div className="now-line" style={{top: topPx}}>
      <span className="now-label">{minutesToSlotLabel(mins)}</span>
    </div>
  );
}
...
/* ─────────────────────────────────────────
   SCHEDULE GRID
───────────────────────────────────────── */
function ScheduleGrid({ dayAppts, providers, onPillClick, onReschedule, onSlotClick, showToast, isToday }: {
  ...
  function handleDragEnd(){setDragApptId(null);setHoverCell(null);}
  function handleFreeDrop(e:React.DragEvent,provider:string,idx:number){
    ...
    if(!valid){showToast('Cannot drop here — slot occupied','error');setDragApptId(null);setHoverCell(null);return;}
    onReschedule(draggedAppt,provider,TIME_SLOTS[idx]);
  ...
  return (
    <div style={{overflowX:'auto',overflowY:'auto',flex:1,WebkitOverflowScrolling:'touch',position:'relative'}}>
      <NowLine isToday={isToday}/>
      <table className="sch-tbl" style={{minWidth:providers.length*160+76}}>
        <thead>
          <tr>
            <th>Time</th>
            {providers.map(p=>(
              <th key={p}>
                <div style={{display:'flex',alignItems:'center',gap:5}}>
                  <div style={{width:24,height:24,borderRadius:'50%',background:'linear-gradient(135deg,#5a8a92,#3a6a72)',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontWeight:700,fontSize:11,flexShrink:0}}>
                  ...
                  if(cell&&cell.isStart){
                    ...
                    return(
                      <td key={p} rowSpan={cell.span} className="occ-cell"
                        ...
                        onDrop={e=>{e.preventDefault();if(dragApptId&&a.bookingID!==dragApptId){showToast('Cannot drop — slot already occupied','error');}setDragApptId(null);setHoverCell(null);}}>
                        <button className={pillCls} draggable={!locked}
                          onDragStart={e=>handleDragStart(e,a)} onDragEnd={handleDragEnd}
                          onClick={()=>{if(!dragApptId)onPillClick(a);}}
                          title={locked?'Ongoing appointments are locked':'Drag to reschedule · Click for details'}>
                          <span className="ap-service">{a.status==='ongoing'&&<span className="live-dot-blue"/>}{locked&&<Ico.Lock s={10}/>}{a.serviceName.split(',')[0]}</span>
                          <span className="ap-time">{a.timeSlot} – {endLabel}</span>
                          <span className="ap-client">{a.clientName}</span>
                          <span className="ap-meta">
                            <MiniModeBadge mode={a.mode}/>
                            <span className="mini-badge" style={{background:'rgba(0,0,0,0.06)',color:'inherit'}}>LKR {a.price.toLocaleString()}</span>
                          </span>
                        </button>
                      </td>
                    );
  ...
  return (
    <div className="appt-card" onClick={onClick}>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
        <span style={{display:'flex',alignItems:'center',gap:3,fontSize:12,color:'#374151',fontWeight:600}}><Ico.Clock/>{appt.timeSlot} – {endLabel}</span>
        <span style={{display:'flex',alignItems:'center',gap:3,fontSize:12,color:'#6b7280'}}><Ico.Loc/>{appt.location}</span>
        <ModeBadge mode={appt.mode}/>
        {appt.guests.length>1&&<span style={{fontSize:11,color:'#6b7280',display:'flex',alignItems:'center',gap:3}}><Ico.Users/>{appt.guests.length} guests</span>}
      </div>
...
export default function AppointmentsPage() {
  ...
  const [filterMeta, setFilterMeta] = useState<FilterMeta>({
    locations: [], categories: [], bookingTypes: [], technicians: [],
  });
  ...
  useEffect(()=>{
    fetch('/api/appointments?meta=filters')
      .then(r=>r.json())
      .then(j=>{
        if(j.success) setFilterMeta({
          locations:    j.locations    || [],
          categories:   j.categories   || [],
          bookingTypes: j.bookingTypes || [],
  ...
  const fetchAppts = useCallback(async(d:string, silent=false)=>{
    ...
    try{
      ...
      if(json.success){
        const seen=new Set<string>();
        const deduped=(json.data as Appointment[]).filter(a=>{
          if(seen.has(a.bookingID)) return false;
          seen.add(a.bookingID); return true;
        });
        setAppts(deduped);
  ...
  useEffect(()=>{fetchAppts(date);},[date,filterLoc]);

  useEffect(()=>{
    if(refreshTimerRef.current) clearInterval(refreshTimerRef.current);
    refreshTimerRef.current=setInterval(()=>fetchAppts(date,true),60000);
    return()=>{if(refreshTimerRef.current)clearInterval(refreshTimerRef.current);};
  },[date,fetchAppts]);

  const handleStatusChange = useCallback(async(bookingID:string,locCode:string,status:Appointment['status'])=>{
    setAppts(prev=>prev.map(a=>a.bookingID===bookingID?{...a,status}:a));
    const labels:Record<string,string>={confirmed:'Appointment confirmed',cancelled:'Appointment cancelled',ongoing:'Client checked in'};
    showToast(labels[status]||'Status updated',status==='cancelled'?'error':'success');
    try{
      const res=await fetch('/api/appointments',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({bookingID,locCode,status})});
      const json=await res.json();
      if(!json.success){fetchAppts(date,true);showToast('Failed to update status','error');}
    } catch {fetchAppts(date,true);showToast('Network error — status not saved','error');}
  },[date,fetchAppts,showToast]);

  const handleDragReschedule = useCallback(async(appt:Appointment,newProvider:string,newTimeSlot:string)=>{
    setAppts(prev=>prev.map(a=>a.bookingID===appt.bookingID?{...a,providerName:newProvider,techID:newProvider,timeSlot:newTimeSlot}:a));
    showToast(`Rescheduled to ${newTimeSlot} · ${newProvider}`,'success');
    try{
      const res=await fetch('/api/appointments',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({bookingID:appt.bookingID,locCode:appt.locCode,date:appt.date,timeSlot:newTimeSlot,techID:newProvider})});
      const json=await res.json();
      if(!json.success){fetchAppts(date,true);showToast('Failed to save reschedule','error');}
    } catch {fetchAppts(date,true);showToast('Network error — reschedule not saved','error');}
  },[date,fetchAppts,showToast]);

  /* ────────────────────────────────────────────────────────
     FIX 1 — Grid slot click → pass date + location too
  ──────────────────────────────────────────────────────── */
  const handleSlotClick = useCallback((provider: string, slotIdx: number) => {
    const params = new URLSearchParams({
      date,                          // ← current grid date
      timeSlot:    TIME_SLOTS[slotIdx],
      providerName: provider,
      techID:       provider,        // form can resolve ID from name
      location:    filterLoc !== 'ALL' ? filterLoc : '',   // branch / locCode
    });
    router.push(`/appointmentform?${params.toString()}`);
  }, [date, filterLoc, router]);

  /* ────────────────────────────────────────────────────────
     FIX 2 — Reschedule nav → pass ALL fields including email
  ──────────────────────────────────────────────────────── */
  const handleRescheduleNav = useCallback((appt: Appointment) => {
    const params = new URLSearchParams({
      ...
      cusCode:      appt.cusCode,
      clientName:   appt.clientName,
      clientPhone:  appt.clientPhone,
      clientEmail:  appt.clientEmail  || '',   // ← EMAIL included
      providerName: appt.providerName,
      techID:       appt.techID,
      serviceName:  appt.serviceName,
      serviceNames: appt.serviceNames.join('||'),
      date:         appt.date,
      timeSlot:     appt.timeSlot,
      duration:     String(appt.duration),
      price:        String(appt.price),
      location:     appt.location,
      locCode2:     appt.locCode,    // alias so form can use either key
      status:       appt.status,
      mode:         appt.mode,
      gender:       appt.gender      || '',
      notes:        appt.notes       || '',
  ...
  const filtered = useMemo(()=>appts.filter(a=>{
    if(filterLoc!=='ALL' && a.locCode!==filterLoc) return false;
    if(filterMode!=='ALL'){
      const selectedType=filterMeta.bookingTypes.find(b=>b.BooikingTypeID===filterMode);
      if(selectedType){
        const des=selectedType.BookingTypeDes.toLowerCase();
        const isWalkIn=des.includes('walk')||des.includes('wi')||des.includes('without');
        if(isWalkIn && a.mode!=='without_confirmation') return false;
        if(!isWalkIn && a.mode!=='pre_booked')          return false;
      }
    }
    if(filterStatus!=='ALL' && a.status!==filterStatus) return false;
    if(filterTech!=='ALL'){
      const techName=filterMeta.technicians.find(t=>t.UserId===filterTech)?.UserName||'';
      if(a.providerName!==techName && a.techID!==filterTech) return false;
    }
    if(search && !`${a.clientName} ${a.clientPhone} ${a.serviceName} ${a.providerName}`.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }),[appts,filterLoc,filterMode,filterStatus,filterTech,filterMeta,search]);
  ...
  const providers = useMemo(()=>{
    const activeProviders = new Set(filtered.map(a=>a.providerName).filter(Boolean));
    const allTechs = filterMeta.technicians.map(t=>t.UserName).filter(Boolean);
    const combined = new Set([...Array.from(activeProviders), ...allTechs]);
    return Array.from(combined).sort();
  },[filtered,filterMeta.technicians]);

  const hasActiveFilters = filterLoc!=='ALL'||filterMode!=='ALL'||filterCat!=='ALL'||filterStatus!=='ALL'||filterTech!=='ALL';

  function pickPeriod(days: number) {
    setDate(shiftDate(todayISO(), days));
  }
    ...
    <>
      ...
      {selected&&(
        <DetailModal
          ...
          onStatusChange={(id,loc,status)=>{setSelected(null);handleStatusChange(id,loc,status);}}
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,overflow:'hidden'}}>
          ...
          <header style={{background:HDR,height:56,flexShrink:0,display:'flex',alignItems:'center',padding:'0 18px',gap:12,borderBottom:'1px solid rgba(0,0,0,0.06)',zIndex:10}}>
            ...
            <div ref={notifRef} style={{position:'relative'}}>
              <button style={{background:'none',border:'none',cursor:'pointer',color:'#374151',display:'flex',alignItems:'center',padding:4,borderRadius:8,position:'relative'}} onClick={()=>{setNotifOpen(o=>!o);setProfileOpen(false);}}>
                <Ico.Bell/>
            ...
            <div ref={profileRef} style={{position:'relative',display:'flex',alignItems:'center',gap:8}}>
              <div className="hdr-name" style={{display:'flex',alignItems:'center',gap:4,cursor:'pointer'}} onClick={()=>{setProfileOpen(o=>!o);setNotifOpen(false);}}>
                <span style={{fontSize:14,fontWeight:500,color:'#1f2937'}}>MR. SAYO</span><Ico.ChevD/>
              </div>
              <div style={{width:34,height:34,borderRadius:'50%',background:'linear-gradient(135deg,#5a8a92,#3a6a72)',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontWeight:700,fontSize:14,cursor:'pointer',flexShrink:0}} onClick={()=>{setProfileOpen(o=>!o);setNotifOpen(false);}}>S</div>
          ...
          <div className="main-body" style={{flex:1,overflow:'auto',padding:'13px 15px',display:'flex',flexDirection:'column',gap:12}}>
            ...
            <div className="filters-row" style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
              <select className="f-sel" value={filterCat} onChange={e=>setFilterCat(e.target.value)}>
                <option value="ALL">All Services</option>
                {filterMeta.categories.map(c=>(
                  <option key={c.CatCode} value={c.CatCode}>{c.CatDes}</option>
                ))}
              </select>
              <select className="f-sel" value={filterLoc} onChange={e=>setFilterLoc(e.target.value)}>
                <option value="ALL">All Branches</option>
                {filterMeta.locations.map(l=>(
                  <option key={l.LocCode} value={l.LocCode}>{l.LocDes}</option>
                ))}
              </select>
              <select className="f-sel" value={filterMode} onChange={e=>setFilterMode(e.target.value)}>
                <option value="ALL">All Modes</option>
                {filterMeta.bookingTypes.map(b=>(
                  <option key={b.BooikingTypeID} value={b.BooikingTypeID}>{b.BookingTypeDes}</option>
                ))}
              </select>
              ...
              <select className="f-sel" value={filterTech} onChange={e=>setFilterTech(e.target.value)}>
                <option value="ALL">All Technicians</option>
                {filterMeta.technicians.map(t=>(
                  <option key={t.UserId} value={t.UserId}>{t.UserName}</option>
                ))}
              </select>
              {hasActiveFilters&&(
                <button onClick={()=>{setFilterLoc('ALL');setFilterMode('ALL');setFilterCat('ALL');setFilterStatus('ALL');setFilterTech('ALL');}}
              ...
              <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:8}}>
                {lastFetch&&<span style={{fontSize:11,color:'#9ca3af'}}>Updated {fmtRelTime(lastFetch.toISOString())}</span>}
                <button className={`refresh-btn ${refreshing?'spinning':''}`} onClick={()=>fetchAppts(date,true)}>
                  <Ico.Refresh s={14} spin={refreshing}/>{refreshing?'Refreshing...':'Refresh'}
                </button>
              </div>
            </div>
            ...
            <div style={{flex:1,minHeight:0,background:PANEL,borderRadius:12,boxShadow:'0 1px 5px rgba(0,0,0,0.08)',overflow:'hidden',display:'flex',flexDirection:'column'}}>
              ...
              <div className="toolbar-row" style={{display:'flex',alignItems:'center',padding:'11px 14px 0',gap:8,flexWrap:'wrap',flexShrink:0}}>
                ...
                {isToday&&(
                  <div style={{display:'flex',alignItems:'center',gap:5,background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.3)',borderRadius:8,padding:'4px 10px'}}>
                    <span style={{width:6,height:6,borderRadius:'50%',background:'#ef4444',animation:'timelinePulse 2s ease-in-out infinite',display:'inline-block'}}/>
                    <span style={{fontSize:11,fontWeight:700,color:'#dc2626',fontFamily:"'Inter',sans-serif"}}>LIVE</span>
                  </div>
                )}
                <div style={{flex:1}}/>
                <div style={{background:'#ccd8d8',borderRadius:9,padding:'3px 4px',display:'flex',gap:2,flexShrink:0}}>
                  ...
                  ] as {label:string;days:number;key:string}[]).map(({label,days,key})=>(
                    <button key={key} className={`period-tab ${period===key?'active':''}`} onClick={()=>pickPeriod(days)}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              {loading?(
                ...
                <div className="fade-up" style={{flex:1,overflow:'auto',padding:'12px 13px'}}>
                  {filtered.length===0?(
                    <div className="empty-state">
                      <div className="empty-ico"><Ico.Inbox/></div>
                      <p style={{fontSize:14,fontWeight:600,color:'#6b7280'}}>{search?'No results found':'No appointments for this date'}</p>
                      <p style={{fontSize:12,color:'#9ca3af'}}>{search?'Try different search terms':'Create a new booking to get started'}</p>
                    </div>
                  ):(
                    <div style={{display:'flex',flexDirection:'column',gap:9}}>
                      {filtered.map((a,idx)=>(
                        <ApptCard key={`${a.bookingID}-${idx}`} appt={a} onClick={()=>setSelected(a)}/>
                      ))}
                    </div>
                  )}
                </div>
