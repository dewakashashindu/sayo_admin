// E:\sayo_admin\sayo-admin\src\app\service\page.tsx
'use client';
...
function canHaveRecipe(it: { serviceItem: boolean; semiFinishedProd: boolean }) {
...
function allowedIngredients(
  ...
  if (menuType === 'service') return allRaw;               // semi + raw
  if (menuType === 'semi')    return allRaw.filter(r => !r.isSemiFinished); // raw only
  return [];
}
...
function emptyItem(): Item {
  ...
  return {
    id: 0, locCode: '', itemCode: '',
    serviceItem: false, semiFinishedProd: false,
    itemDes: '', itemPrintDes: '', masterUnitID: 'UNT03',
    ...
    itemPic: null, createDate: today, createBy: 'ADMIN',
    updDate: today, updBy: 'ADMIN', enable: true, locationDetails: [],
  };
}
...
const PAGE_CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .frm-select {
    width:100%; border:1.5px solid #d1d9da; border-radius:8px;
    padding:0 28px 0 11px; height:36px; font-family:'Inter',sans-serif;
...
const IGrid    = ({s=14}:{s?:number})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>;
const IClose   = ({s=18}:{s?:number})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>;
const IZap     = ({s=14}:{s?:number})=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>;
...
function Card({ id, title, icon, badge, children }: {
  id?:string; title:string; icon?:React.ReactNode; badge?:string; children:React.ReactNode;
}) {
  return (
...
function TypeBadge({ it }: { it:{ serviceItem:boolean; semiFinishedProd:boolean } }) {
  const t = itemType(it);
  if (t==='service') return <span className="badge-service">Service</span>;
  if (t==='semi')    return <span className="badge-semi">Semi-Finished</span>;
...
function Hl({ text, q }: { text:string; q:string }): React.ReactElement {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark style={{ background:'#fef08a', color:'#1e3a40', borderRadius:2, padding:'0 1px', fontWeight:700 }}>
...
const FLAG_DEFS: FlagDef[] = [
  ...
  { key:'semiFinishedProd', label:'Semi-Finished Prod', subtitle:'Intermediate production item',  color:'#be185d' },
...
function FlagsCard({ item, onChange }: {
  ...
  function toggle(def: FlagDef) {
    const newVal = !Boolean(item[def.key]);
    if (def.key==='serviceItem'      && newVal) onChange('semiFinishedProd', false as Item['semiFinishedProd']);
    if (def.key==='semiFinishedProd' && newVal) onChange('serviceItem',      false as Item['serviceItem']);
    onChange(def.key, newVal as Item[typeof def.key]);
  }
  return (
    <Card id="sec-flags" title="Flags" icon={<IFlag s={13}/>} badge={`${activeCount} / ${FLAG_DEFS.length} active`}>
      <div className="flags-grid">
        {FLAG_DEFS.map(def => {
          ...
          return (
            <div key={String(def.key)} className={`flag-card ${isOn?'is-on':''}`}
              style={{ '--fc':def.color } as React.CSSProperties}>
              <div className="flag-card-inner">
                <div className="flag-toggle-row" role="checkbox" aria-checked={isOn} tabIndex={0}
                  onClick={()=>toggle(def)}
                  onKeyDown={e=>{ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); toggle(def); } }}>
                ...
                <div className="flag-sub-area">
                  {def.subInputs.map(inp => (
                    <div key={String(inp.field)}>
                      ...
                      <input className="flag-sub-input" type="number" min={0}
                        value={item[inp.field] as number} disabled={!isOn}
                        style={{ '--fc':def.color } as React.CSSProperties}
...
/* ═══════════════════════════════════════════════════════
   ITEM CODE INPUT  (main form — autocomplete existing items)
═══════════════════════════════════════════════════════ */
function ItemCodeInput({ value, allItems, isNew, onSelect, onChange }: {
  value:    string;
  allItems: Item[];
  isNew:    boolean;
  ...
  const suggestions = useMemo(() => {
    ...
    return allItems.filter(it =>
      it.itemCode.toUpperCase().includes(q) || it.itemDes.toUpperCase().includes(q)
    ).slice(0, 10);
  ...
  const updatePos = useCallback(() => {
    ...
    setDdStyle({ top: r.bottom + 4, left: r.left, width: Math.max(r.width, 360), maxHeight: 300 });
  }, []);

  useEffect(() => { if (open) updatePos(); }, [open, updatePos, suggestions.length]);
  ...
  if (!isNew) return <input className="frm-input" value={value} readOnly/>;

  return (
    <>
      <input ref={inputRef} className="frm-input" style={{ textTransform:'uppercase' }}
        value={value} placeholder="Enter new or existing code…" autoComplete="off" maxLength={15}
        onChange={e=>{ onChange(e.target.value.toUpperCase()); setOpen(true); setHiIdx(0); }}
        onFocus={()=>{ if(value.trim()){ setOpen(true); updatePos(); } }}
        onKeyDown={e=>{
          if (e.key==='ArrowDown'){ e.preventDefault(); setOpen(true); setHiIdx(h=>Math.min(h+1,suggestions.length-1)); }
          else if (e.key==='ArrowUp'){ e.preventDefault(); setHiIdx(h=>Math.max(h-1,0)); }
          else if (e.key==='Enter'){ if(open&&suggestions[hiIdx]){ e.preventDefault(); onSelect(suggestions[hiIdx]); setOpen(false); } }
          else if (e.key==='Escape'||e.key==='Tab'){ setOpen(false); }
        }}/>
      {open && suggestions.length > 0 && typeof document !== 'undefined' && (() => {
        const { createPortal } = require('react-dom');
        return createPortal(
          <div ref={ddRef} className="itm-ac-dropdown" style={{ position:'fixed', ...ddStyle }}
            onMouseDown={e=>e.preventDefault()}>
            {suggestions.map((it,i) => (
              <div key={`${it.locCode}|${it.itemCode}`}
                className={`itm-ac-item ${i===hiIdx?'hi':''}`}
                onMouseDown={()=>{ onSelect(it); setOpen(false); }}
                onMouseEnter={()=>setHiIdx(i)}>
                <div className="itm-ac-thumb">
                  {it.itemPic
                    ? <img src={it.itemPic} alt="" style={{ width:'100%',height:'100%',objectFit:'cover' }}/>
                    : <IBox s={15}/>}
                </div>
                <div className="itm-ac-main">
                  <div style={{ display:'flex',alignItems:'center',gap:6 }}>
                    <span className="itm-ac-code"><Hl text={it.itemCode} q={value}/></span>
                    <TypeBadge it={it}/>
                  </div>
                  <div className="itm-ac-des"><Hl text={it.itemDes} q={value}/></div>
                  <div className="itm-ac-sub">Loc: {it.locCode}</div>
                </div>
                <span className="itm-ac-price">LKR {it.retailPrice.toFixed(0)}</span>
              </div>
            ))}
          </div>,
          document.body
...
/* ═══════════════════════════════════════════════════════
   INGREDIENT AUTOCOMPLETE  (recipe rows — code OR description)
═══════════════════════════════════════════════════════ */
function IngredientAC({ value, items, placeholder, onSelect, onChange }: {
  value:        string;
  items:        RawItem[];
  placeholder?: string;
  onSelect:     (it:RawItem) => void;
  ...
  const suggestions = useMemo(() => {
    ...
    return items.filter(it =>
      it.code.toUpperCase().includes(q) || it.des.toUpperCase().includes(q)
    ).slice(0, 14);
  }, [value, items]);
  ...
  const updatePos = useCallback(() => {
    ...
    const dropH = Math.min(320, suggestions.length * 56 + 8);
    if (spaceBelow >= dropH || spaceBelow >= spaceAbove) {
      setDdStyle({ top:r.bottom+window.scrollY+4, left:r.left+window.scrollX, width:Math.max(r.width,360), maxHeight:Math.min(320,spaceBelow-8) });
    } else {
      setDdStyle({ top:r.top+window.scrollY-dropH-4, left:r.left+window.scrollX, width:Math.max(r.width,360), maxHeight:Math.min(320,spaceAbove-8) });
    }
  }, [suggestions.length]);
  ...
  return (
    <>
      <input ref={inputRef} className="rcp-input"
        value={value} placeholder={placeholder ?? 'Code / Name…'} autoComplete="off"
        onChange={e=>{ onChange(e.target.value); setOpen(true); setHiIdx(0); }}
        onFocus={()=>{ if(value.trim()){ setOpen(true); updatePos(); } }}
        onKeyDown={e=>{
          if (e.key==='ArrowDown'){ e.preventDefault(); setOpen(true); setHiIdx(h=>Math.min(h+1,suggestions.length-1)); }
          else if (e.key==='ArrowUp'){ e.preventDefault(); setHiIdx(h=>Math.max(h-1,0)); }
          else if (e.key==='Enter'){ if(open&&suggestions[hiIdx]){ e.preventDefault(); e.stopPropagation(); onSelect(suggestions[hiIdx]); setOpen(false); } }
          else if (e.key==='Escape'||e.key==='Tab'){ setOpen(false); }
        }}/>
      ...
      {show && typeof document !== 'undefined' && (() => {
        const { createPortal } = require('react-dom');
        return createPortal(
          <div ref={ddRef} className="ac-portal-dropdown" style={ddStyle}
            onMouseDown={e=>e.preventDefault()}>
            {suggestions.map((it,i) => (
              <div key={it.code} className={`ac-item ${i===hiIdx?'highlighted':''}`}
                style={{ padding:'10px 13px' }}
                onMouseDown={()=>{ onSelect(it); setOpen(false); }}
                onMouseEnter={()=>setHiIdx(i)}>
                {/* Code */}
                <span className="ac-code"><Hl text={it.code} q={value}/></span>
                {/* Des + type */}
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:12.5, fontWeight:600, color:'#1f2937', lineHeight:1.35 }}>
                    <Hl text={it.des} q={value}/>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:3 }}>
                    {it.isSemiFinished
                      ? <span className="badge-semi" style={{ fontSize:9 }}>Semi-Finished</span>
                      : <span className="badge-raw"  style={{ fontSize:9 }}>Raw Item</span>}
                    <span style={{ fontSize:10.5, color:'#6b7280' }}>
                      Unit: <strong style={{ color:'#1e3a40' }}>{it.unit}</strong>
                    </span>
                  </div>
                </div>
                {/* Cost */}
...
/* ═══════════════════════════════════════════════════════
   LOCATION GRID
═══════════════════════════════════════════════════════ */
function LocationGrid({ rows, onChange }: { rows:LocationDetail[]; onChange:(u:LocationDetail[])=>void }) {
  ...
  if (rows.length===0)
    return <p style={{ color:'#9ca3af',fontSize:13,padding:'16px 0',textAlign:'center' }}>Loading locations…</p>;
  return (
      ...
      <table className="loc-grid">
        <thead>
          <tr>
            ...
            <th>Sales Margin %</th><th>Retail Price</th><th>WS Price</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row,idx) => (
            <tr key={row.locCode}>
              ...
              <td>
                <input className="loc-grid-input" value={row.locStockBalance.toFixed(2)} readOnly
...
/* ═══════════════════════════════════════════════════════
   RECIPE GRID  — both Code & Description columns searchable
   ✅ FIX: colgroup không còn khoảng trắng/comment giữa các <col>
═══════════════════════════════════════════════════════ */
function RecipeGrid({ rows, allowedItems, subUnits, onChange, menuItmID, defaultLocCode }: {
  rows:           RecipeRow[];
  ...
  /* When code AC selects → fill all */
  function selectByCode(idx:number, it:RawItem) {
    onChange(rows.map((r,i) => i!==idx ? r : {
      ...r, rowItemCode:it.code, rowItemDes:it.des, masterUnitID:it.unit, itemCost:it.cost,
    }));
  }
  /* When description AC selects → fill all */
  function selectByDes(idx:number, it:RawItem) {
    onChange(rows.map((r,i) => i!==idx ? r : {
      ...r, rowItemCode:it.code, rowItemDes:it.des, masterUnitID:it.unit, itemCost:it.cost,
    }));
  }
  ...
  function updRow(idx:number, key:keyof RecipeRow, val:RecipeRow[keyof RecipeRow]) {
    onChange(rows.map((r,i) => {
      if (i!==idx) return r;
      const u:RecipeRow = { ...r,[key]:val };
      /* If code typed manually and exact match → auto fill */
      if (key==='rowItemCode') {
        const q = String(val).trim().toUpperCase();
        const found = allowedItems.find(it=>it.code.trim().toUpperCase()===q);
        if (found){ u.rowItemDes=found.des; u.masterUnitID=found.unit; u.itemCost=found.cost; }
      }
      return u;
    }));
  }
  /* Description typed freely */
  function updDes(idx:number, val:string) {
    onChange(rows.map((r,i) => i!==idx ? r : { ...r, rowItemDes:val }));
  }
  ...
  return (
    <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
      <div className="rcp-wrap" style={{ overflowX:'auto',overflowY:'visible' }}>
        <table className="rcp-table" style={{ tableLayout:'fixed',minWidth:840 }}>
          <colgroup>
            <col style={{ width:36  }}/>
            <col style={{ width:160 }}/>
          ...
          <tbody>
            ...
            {rows.map((row,idx) => (
              <tr key={idx}>
                ...
                {/* Code column — AC searches code + description */}
                <td>
                  <IngredientAC
                    value={row.rowItemCode}
                    items={allowedItems}
                    placeholder="Code…"
                    onChange={val => updRow(idx,'rowItemCode',val.toUpperCase())}
                    onSelect={it  => selectByCode(idx,it)}
                  />
                </td>

                {/* Description column — AC also searches code + description */}
                <td>
                  <IngredientAC
                    value={row.rowItemDes}
                    items={allowedItems}
                    placeholder="Type description to search…"
                    onChange={val => updDes(idx,val)}
                    onSelect={it  => selectByDes(idx,it)}
                  />
                </td>

                {/* Sub Unit */}
                <td>
                  <select className="rcp-input frm-select" style={{ paddingRight:28 }}
                    value={row.subUnitID} onChange={e=>updRow(idx,'subUnitID',e.target.value)}>
                ...
                <td>
                  <button className="rcp-btn-remove"
                    onClick={()=>onChange(rows.filter((_,i)=>i!==idx))} title="Remove">
...
/* ═══════════════════════════════════════════════════════
   ITEMS POPUP
═══════════════════════════════════════════════════════ */
function ItemsPopup({ items, locations, category1, onSelect, onClose }: {
  items:Item[]; locations:MasterOpt[]; category1:MasterOpt[];
  onSelect:(it:Item)=>void; onClose:()=>void;
}) {
  ...
  const filtered = useMemo(() => items.filter(it => {
    ...
    const matchQ = !q || it.itemDes.toLowerCase().includes(q) || it.itemCode.toLowerCase().includes(q);
    const matchL = !filterLoc  || it.locCode===filterLoc;
    const matchC = !filterCat  || it.category1===filterCat;
    const matchE = filterEn==='' || (filterEn==='1' ? it.enable : !it.enable);
    const matchT = !filterType || itemType(it)===filterType;
    return matchQ && matchL && matchC && matchE && matchT;
  }), [items,search,filterLoc,filterCat,filterEn,filterType]);
  ...
  return (
    <div className="items-modal-overlay" onClick={e=>{ if(e.target===e.currentTarget) onClose(); }}>
      <div className="items-modal">
        {/* Header */}
        <div className="items-modal-hdr">
          <div style={{ width:38,height:38,borderRadius:10,background:'rgba(255,255,255,0.15)',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',flexShrink:0 }}>
            <IGrid s={18}/>
          </div>
          <div style={{ flex:1 }}>
            <p style={{ color:'rgba(255,255,255,0.5)',fontSize:10,fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase' }}>Item Master</p>
            <p style={{ color:'#fff',fontSize:16,fontWeight:800 }}>
              All Items &nbsp;<span style={{ color:'rgba(255,255,255,0.4)',fontSize:13,fontWeight:500 }}>({filtered.length} / {items.length})</span>
            </p>
          </div>
        ...
        <div className="items-modal-filters">
          <div style={{ position:'relative',flex:1,minWidth:180 }}>
            <span style={{ position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',opacity:0.4,display:'flex',alignItems:'center',pointerEvents:'none' }}><ISearch s={14}/></span>
            <input className="popup-filter-input" style={{ width:'100%',paddingLeft:32 }}
              placeholder="Search code or name…" value={search}
              onChange={e=>setSearch(e.target.value)} autoFocus/>
          </div>
          <select className="popup-filter-select" style={{ minWidth:150 }} value={filterLoc} onChange={e=>setFilterLoc(e.target.value)}>
            <option value="">All Locations</option>
            {locations.map(l=><option key={l.code} value={l.code}>{l.code} – {l.name}</option>)}
          </select>
          <select className="popup-filter-select" style={{ minWidth:140 }} value={filterCat} onChange={e=>setFilterCat(e.target.value)}>
            <option value="">All Categories</option>
            {category1.map(c=><option key={c.code} value={c.code}>{c.des}</option>)}
          </select>
          <select className="popup-filter-select" style={{ minWidth:145 }} value={filterType} onChange={e=>setFilterType(e.target.value)}>
            <option value="">All Types</option>
            <option value="service">Service Items</option>
            <option value="semi">Semi-Finished</option>
            <option value="raw">Raw Items</option>
          </select>
          <select className="popup-filter-select" style={{ minWidth:110 }} value={filterEn} onChange={e=>setFilterEn(e.target.value)}>
            <option value="">All Status</option>
            <option value="1">Active</option>
            <option value="0">Inactive</option>
          </select>
          {(search||filterLoc||filterCat||filterEn||filterType) && (
            <button onClick={()=>{setSearch('');setFilterLoc('');setFilterCat('');setFilterEn('');setFilterType('');}}
              style={{ padding:'0 12px',height:36,borderRadius:8,border:'1.5px solid #d1d9da',background:'#f3f4f6',color:'#374151',fontFamily:"'Inter',sans-serif",fontSize:12,fontWeight:700,cursor:'pointer' }}>
              Clear
            </button>
          )}
        </div>
        ...
        <div className="items-modal-list">
          {filtered.length===0 && (
            <div style={{ textAlign:'center',padding:'48px 0',color:'#9ca3af' }}>
              <IBox s={40}/><p style={{ marginTop:12,fontSize:14,fontWeight:600 }}>No items match</p>
            </div>
          )}
          {filtered.map(it=>(
            <button key={`${it.locCode}|${it.itemCode}`} className="popup-item-row"
              onClick={()=>{ onSelect(it); onClose(); }}>
              <div style={{ width:42,height:42,borderRadius:9,background:it.enable?'linear-gradient(135deg,#1e3a40,#2a5260)':'#d1d5db',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',flexShrink:0,overflow:'hidden' }}>
                {it.itemPic?<img src={it.itemPic} alt="" style={{ width:'100%',height:'100%',objectFit:'cover' }}/>:<IBox s={18}/>}
              </div>
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ display:'flex',alignItems:'center',gap:8,flexWrap:'wrap' }}>
                  <span style={{ fontSize:13,fontWeight:800,color:'#1e3a40' }}>{it.itemDes}</span>
                  <span style={{ fontSize:11,fontWeight:600,color:'#6b7280',background:'rgba(30,58,64,0.07)',padding:'1px 7px',borderRadius:5 }}>{it.itemCode}</span>
                </div>
                <div style={{ display:'flex',gap:6,marginTop:5,flexWrap:'wrap',alignItems:'center' }}>
                  <span className={it.enable?'badge-active':'badge-inactive'}>{it.enable?'Active':'Inactive'}</span>
                  <TypeBadge it={it}/>
                  {it.category1&&<span className="badge-cat">{category1.find(c=>c.code===it.category1)?.des??it.category1}</span>}
                  <span style={{ fontSize:11,color:'#9ca3af' }}>Loc: <strong style={{ color:'#1e3a40' }}>{it.locCode}</strong></span>
                </div>
              </div>
              <div style={{ textAlign:'right',flexShrink:0 }}>
                <p style={{ fontSize:13,fontWeight:800,color:'#1e3a40' }}>LKR {it.retailPrice.toLocaleString()}</p>
                <p style={{ fontSize:11,color:'#9ca3af',marginTop:2 }}>Cost: {it.rawCost.toFixed(2)}</p>
                {canHaveRecipe(it)&&<p style={{ fontSize:10,color:'#7c3aed',fontWeight:700,marginTop:2 }}>⚗ Recipe</p>}
              </div>
            </button>
          ))}
        </div>
...
export default function ItemMasterPage() {
  ...
  const loadRecipeMaster = useCallback(async () => {
    try {
      ...
      const json = await res.json() as {
        success:boolean;
        recipes:Array<{menuItmID:string;rowItemCode:string;masterUnitID:string;subUnitID:string;qty:number;locCode:string;itemCost:number}>;
        rawItems:RawItem[]; subUnits:SubUnit[];
      };
      ...
      setAllRecipes(
        (json.recipes??[]).map(r=>{
          ...
          return{ ...r, rowItemDes:found?.des??'', masterUnitID:found?.unit??r.masterUnitID, itemCost:r.itemCost!==0?r.itemCost:(found?.cost??0) };
        })
      );
    } catch{ /* silent */ }
  },[]);
  ...
  /* ── load recipes for current item ── */
  const loadRecipes = useCallback((menuItmID:string, locCode:string) => {
    if (!menuItmID){ setRecipeRows([]); return; }
    setRecipeRows(
      allRecipes
        .filter(r=>r.menuItmID.trim().toUpperCase()===menuItmID.trim().toUpperCase()&&r.locCode.trim().toUpperCase()===locCode.trim().toUpperCase())
        .map(r=>({...r}))
    );
  },[allRecipes]);

  useEffect(()=>{
    if (!isNew&&current.itemCode&&hasRecipeRights) loadRecipes(current.itemCode,primaryLocCode);
    else setRecipeRows([]);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[current.itemCode,isNew,primaryLocCode,loadRecipes,hasRecipeRights]);

  useEffect(()=>{ if(current.locCode) setRecipeLocCodes([current.locCode]); },[current.locCode]);

  /* ── pre-fill location details for new items ── */
  useEffect(()=>{
    if (!isNew||locations.length===0) return;
    if (current.locationDetails.length>0) return;
    setCurrent(prev=>({
      ...prev,
      locationDetails: locations.map(l=>({
        locCode:l.code, locName:l.name??l.code,
        enable:true, locStockBalance:0, salesMargin:0, retailPrice:0, wsPrice:0,
      })),
    }));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  },[isNew,locations]);
  ...
  /* ── helpers ── */
  function upd<K extends keyof Item>(key:K, val:Item[K]){ setCurrent(p=>({...p,[key]:val})); }
  function updLocDetail(u:LocationDetail[]){ setCurrent(p=>({...p,locationDetails:u})); }

  /* ── new item ── */
  function handleNew() {
    const fresh = emptyItem();
    fresh.locationDetails = locations.map(l=>({
      locCode:l.code, locName:l.name??l.code,
      enable:true, locStockBalance:0, salesMargin:0, retailPrice:0, wsPrice:0,
    }));
    setCurrent(fresh);
  ...
  /* ── select from popup ── */
  function handleSelect(it:Item){ setCurrent({...it}); setIsNew(false); setActiveTab('details'); }

  /* ── auto-fill when existing code typed ── */
  function handleItemCodeSelect(it:Item){ setCurrent({...it}); setIsNew(false); }

  /* ── save ── */
  const handleSave = useCallback(async () => {
    if (!current.itemDes.trim())  { showToast('Item Description is required',true); return; }
    if (!current.itemCode.trim()) { showToast('Item Code is required',true); return; }
    setSaving(true);
    const savedLocCode  = current.locCode.trim()||(locations[0]?.code??'01');
    const savedItemCode = current.itemCode.trim().toUpperCase();
    try {
      const payload = { ...current, locCode:savedLocCode, itemCode:savedItemCode };
      const res = isNew
        ? await fetch('/api/services',{ method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload) })
        : await fetch(`/api/services/${encodeURIComponent(savedLocCode)}/${encodeURIComponent(savedItemCode)}`,
      ...
      if (isNew){
        setRecentItems(prev=>{
          const ne={...current,itemCode:savedItemCode,locCode:savedLocCode};
          return [ne,...prev.filter(p=>p.itemCode!==savedItemCode)].slice(0,3);
        });
      }
      await loadItems();
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res  = await fetch(`/api/services/${encodeURIComponent(current.locCode)}/${encodeURIComponent(current.itemCode)}`,{ method:'DELETE' });
      const json = await res.json() as{ success:boolean; message?:string };
  ...
  /* ── reset ── */
  function handleClear(){ if(isNew){ handleNew(); return; } const orig=items.find(s=>s.id===current.id); if(orig) setCurrent({...orig}); }
  ...
  const handleSaveRecipes = useCallback(async () => {
    ...
    try {
      const lines=recipeRows.map(r=>({ rowItemCode:r.rowItemCode,masterUnitID:r.masterUnitID,subUnitID:r.subUnitID,qty:r.qty,itemCost:r.itemCost }));
      await Promise.all(
        recipeLocCodes.map(lc=>fetch('/api/recipes',{
      ...
      await loadRecipeMaster();
    } catch(err){ showToast(err instanceof Error?err.message:'Recipe save failed',true); }
  ...
  function handleFormKeyDown(e:React.KeyboardEvent<HTMLDivElement>){
    ...
    const next=focusable[idx+1]; next.focus();
    if (next instanceof HTMLInputElement) next.select();
  }

  /* ── Ctrl+S ── */
  const saveRef=useRef(handleSave); const busyRef=useRef(false);
  saveRef.current=handleSave; busyRef.current=saving||deleting;
  useEffect(()=>{
    function onKey(e:KeyboardEvent){
      if (!((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='s')) return;
      e.preventDefault(); if(!busyRef.current) saveRef.current();
    }
    window.addEventListener('keydown',onKey);
    return ()=>window.removeEventListener('keydown',onKey);
  },[]);
  ...
  return (
    <>
      ...
      {showPopup&&(
        <ItemsPopup items={items} locations={locations} category1={category1}
          onSelect={handleSelect} onClose={()=>setShowPopup(false)}/>
      )}
        ...
        <div style={{ flex:1,display:'flex',flexDirection:'column',minWidth:0,overflow:'hidden' }}>
          ...
          <header style={{ background:'#dae6e6',height:56,flexShrink:0,display:'flex',alignItems:'center',padding:'0 18px',gap:12,borderBottom:'1px solid rgba(0,0,0,0.06)',zIndex:10 }}>
            <div style={{ display:'flex',alignItems:'center',gap:8 }}>
              <span style={{ fontSize:15,fontWeight:800,color:'#1e3a40' }}>Item Master</span>
              <span style={{ fontSize:12,color:'#6b7280',fontWeight:500 }}>{loading?'…':`${items.length} items`}</span>
            </div>
            <div style={{ flex:1 }}/>
          ...
          <div style={{ flex:1,overflow:'hidden',padding:'12px 14px',display:'flex',gap:12,flexDirection:'column' }}>

            {/* ── RECENT STRIP ── */}
            {recentItems.length>0&&(
              <div style={{ flexShrink:0 }}>
                <p style={{ fontSize:11,fontWeight:700,color:'#6b7280',textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:6 }}>Recently Added</p>
                <div className="recent-strip">
                  {recentItems.map(it=>(
                    <button key={`${it.locCode}|${it.itemCode}`} className="recent-card" onClick={()=>handleSelect(it)}>
                      <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                        <div style={{ width:32,height:32,borderRadius:8,background:'linear-gradient(135deg,#1e3a40,#2a5260)',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',flexShrink:0,overflow:'hidden' }}>
                          {it.itemPic?<img src={it.itemPic} alt="" style={{ width:'100%',height:'100%',objectFit:'cover' }}/>:<IBox s={15}/>}
                        </div>
                        <div style={{ flex:1,minWidth:0 }}>
                          <p style={{ fontSize:12,fontWeight:700,color:'#1e3a40',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{it.itemDes}</p>
                          <p style={{ fontSize:10,color:'#6b7280' }}>{it.itemCode}</p>
                        </div>
                      </div>
                      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                        <TypeBadge it={it}/>
                        <span style={{ fontSize:11,fontWeight:700,color:'#1e3a40' }}>LKR {it.retailPrice.toFixed(0)}</span>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {/* ── MAIN PANEL ── */}
            <div style={{ flex:1,overflow:'hidden',display:'flex',flexDirection:'column',borderRadius:14,boxShadow:'0 2px 12px rgba(30,58,64,0.1)' }}>

              {/* Panel header */}
              <div style={{ background:'#1e3a40',borderRadius:'14px 14px 0 0',padding:'12px 18px 0',flexShrink:0 }}>
                <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12,marginBottom:4 }}>
                  <div>
                    <p style={{ color:'rgba(255,255,255,0.4)',fontSize:10,fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase' }}>
                      {isNew?'New Item':'Edit Item'} · Ctrl+S to save
                    </p>
                    <div style={{ display:'flex',alignItems:'center',gap:10,marginTop:2 }}>
                      <p style={{ color:'#fff',fontSize:17,fontWeight:800 }}>ITEM MASTER DETAIL</p>
                      {!isNew&&<TypeBadge it={current}/>}
                    </div>
                  </div>
                  <div style={{ display:'flex',gap:8,paddingTop:2 }}>
                    <button className="btn-new" onClick={handleNew} disabled={busy}><IPlus s={13}/> New Item</button>
                    <button className="btn-viewmore" onClick={()=>setShowPopup(true)}><IGrid s={13}/> Browse</button>
                  </div>
                </div>
                ...
                <div className="panel-tabs">
                  ...
                  {hasRecipeRights&&(
                    <button className={`panel-tab ${activeTab==='recipe'?'active':''}`}
                      onClick={()=>{
                        if(isNew){ showToast('Save item first to manage recipe',true); return; }
                        setActiveTab('recipe');
                        loadRecipes(current.itemCode,primaryLocCode);
                      }}>
                      <IFlask s={13}/> Recipe Management
                      {isNew&&<span style={{ fontSize:9,opacity:0.6,marginLeft:2 }}>(save first)</span>}
                    </button>
                  )}
                </div>
              </div>
              ...
              {activeTab==='details'&&(
                <div className="quick-nav">
                  {SECTIONS.map(s=>(
                    <button key={s.id} className="quick-nav-btn" onClick={()=>scrollTo(s.id)}>{s.icon}{s.label}</button>
                  ))}
                </div>
              )}

              {/* ── BODY ── */}
              <div ref={formRef} onKeyDown={handleFormKeyDown}
                style={{ flex:1,overflowY:'auto',padding:'14px',display:'flex',flexDirection:'column',gap:14,background:'#e8f0f1' }}>
                  ...
                  <div className="fade-up" style={{ display:'flex',flexDirection:'column',gap:14 }}>
                    {!hasRecipeRights?(
                      <div className="recipe-blocked">
                        <IBox s={36}/>
                        <p style={{ fontSize:14,fontWeight:700,color:'#6b7280',marginTop:12 }}>Raw items do not have a recipe.</p>
                        <p style={{ fontSize:12,color:'#9ca3af',marginTop:4 }}>Only <strong>Service Items</strong> and <strong>Semi-Finished Products</strong> can have recipes.</p>
                      </div>
                    ):(
                      <>
                        {/* Recipe banner */}
                        <div style={{ background:'linear-gradient(135deg,#1e3a40,#2a5260)',borderRadius:12,padding:'16px 20px',display:'flex',alignItems:'flex-start',gap:16,flexWrap:'wrap' }}>
                          <div style={{ flex:1,minWidth:200 }}>
                            ...
                            <p style={{ color:'rgba(255,211,100,0.8)',fontSize:10,fontWeight:600,marginTop:6 }}>
                              {currentType==='service'
                          ...
                          <div style={{ display:'flex',flexDirection:'column',gap:8,alignItems:'flex-end' }}>
                            ...
                            {recipeLocCodes.length>1&&(
                              <p style={{ color:'rgba(255,211,100,0.85)',fontSize:10,fontWeight:600 }}>⚠ Recipe will be saved to {recipeLocCodes.length} locations</p>
                            )}
                          </div>
                        </div>
                ...
                {!loading&&activeTab==='details'&&(
                  <div className="fade-up" style={{ display:'flex',flexDirection:'column',gap:14 }}>
                    ...
                    <Card id="sec-ident" title="Item Identification" icon={<ITag s={13}/>}>
                      <div style={{ display:'grid',gridTemplateColumns:'1fr 2fr',gap:12,marginBottom:12 }}>
                        <FieldRow label="Item Code *">
                          <ItemCodeInput
                            value={current.itemCode} allItems={items} isNew={isNew}
                            onSelect={handleItemCodeSelect}
                            onChange={val=>upd('itemCode',val)}/>
                        </FieldRow>
                        <FieldRow label="Item Description *" htmlFor="itm-des">
                          <input id="itm-des" className="frm-input" value={current.itemDes}
                            onChange={e=>upd('itemDes',e.target.value)}
                            placeholder="e.g. Shampoo & Conditioner" maxLength={50}/>
                        </FieldRow>
                      </div>
                      <div style={{ display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:12 }}>
                        <FieldRow label="Print Description" htmlFor="itm-printdes">
                          <input id="itm-printdes" className="frm-input" value={current.itemPrintDes}
                            onChange={e=>upd('itemPrintDes',e.target.value)} maxLength={50}/>
                        </FieldRow>
                        <FieldRow label="Master Unit" htmlFor="itm-unit">
                          <select id="itm-unit" className="frm-select" value={current.masterUnitID}
                            onChange={e=>upd('masterUnitID',e.target.value)}>
                            {units.map(u=><option key={u.id} value={u.id}>{u.des}</option>)}
                          </select>
                        </FieldRow>
                        <FieldRow label="Supplier" htmlFor="itm-sup">
                          <select id="itm-sup" className="frm-select" value={current.supID}
                            onChange={e=>upd('supID',e.target.value)}>
                    ...
                    {isNew&&hasRecipeRights&&(
                      <div style={{ background:'linear-gradient(135deg,#ede9fe,#f5f3ff)',border:'1.5px solid #c4b5fd',borderRadius:10,padding:'12px 16px',display:'flex',alignItems:'center',gap:10 }}>
                        <IZap s={16}/>
                        <div>
                          <p style={{ fontSize:12,fontWeight:700,color:'#7c3aed' }}>
                            {currentType==='service'?'Service Item':'Semi-Finished Product'} — Recipe Available
                          </p>
                          <p style={{ fontSize:11,color:'#6b7280',marginTop:2 }}>
                            Save this item first, then use the <strong>Recipe Management</strong> tab to add ingredients.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <Card id="sec-cost" title="Cost & Margin" icon={<IDollar s={13}/>}>
                      <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr 1fr',gap:12 }}>
                        <FieldRow label="Raw Cost" htmlFor="itm-rawcost">
                          <input id="itm-rawcost" className="frm-input" type="number"
                            value={current.rawCost} onChange={e=>upd('rawCost',Number(e.target.value))} min={0}/>
                        </FieldRow>
                        <FieldRow label="Cost Markup %">
                          <input className="frm-input" type="number"
                            value={current.costMarkup} onChange={e=>upd('costMarkup',Number(e.target.value))} min={0}/>
                        </FieldRow>
                        <FieldRow label="Overall Cost (Auto)">
                          <input className="frm-input" value={overallCost.toFixed(2)} readOnly
                            style={{ background:'#f0fdf4',color:'#15803d',fontWeight:700 }}/>
                        </FieldRow>
                        <FieldRow label="Sales Margin %">
                          <input className="frm-input" type="number"
                            value={current.salesMargin} onChange={e=>upd('salesMargin',Number(e.target.value))} min={0}/>
                        </FieldRow>
                        <FieldRow label="Retail Price">
                          <input className="frm-input" type="number"
                            value={current.retailPrice} onChange={e=>upd('retailPrice',Number(e.target.value))} min={0}/>
                        </FieldRow>
                      </div>
                      <div style={{ marginTop:12,background:'linear-gradient(135deg,#1e3a40,#2a5260)',borderRadius:10,padding:'10px 16px',display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12 }}>
                        {[
                          { label:'Raw Cost',    val:`LKR ${current.rawCost.toLocaleString()}` },
                          { label:'Overall Cost',val:`LKR ${overallCost.toFixed(2)}` },
                          { label:'Retail Price',val:`LKR ${current.retailPrice.toLocaleString()}` },
                        ].map(it=>(
                          <div key={it.label}>
                    ...
                    <Card id="sec-reorder" title="Reorder Levels & Stock Limits" icon={<ILayers s={13}/>}>
                      ...
                      <div style={{ marginTop:10,background:'#fffbeb',border:'1.5px solid #fde68a',borderRadius:8,padding:'8px 12px',display:'flex',alignItems:'center',gap:8 }}>
                        ...
                        <span style={{ fontSize:12,color:'#92400e',fontWeight:500 }}>
                          Stock is sourced from <strong>tbl_itemdetail</strong> per location — shown read-only in Location Details.
                        </span>
                      </div>
                    </Card>

                    {/* Locations */}
                    <Card id="sec-locations" title="Location Details" icon={<IMapPin s={13}/>}
                      badge={`${current.locationDetails.length} locations`}>
                      <p style={{ fontSize:12,color:'#6b7280',marginBottom:10 }}>
                        All active locations. Enable/disable per location, set pricing & margins.
                        Stock is read-only 
                      </p>
                      ...
                      {current.locationDetails.length>0&&(
                        <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:10,marginTop:12 }}>
                          {current.locationDetails.map(ld=>(
                            <div key={ld.locCode} style={{ background:'#fff',borderRadius:10,border:`1.5px solid ${ld.enable?'#bbf7d0':'#fecaca'}`,padding:'10px 12px' }}>
                              ...
                              <p style={{ fontSize:11,color:'#6b7280',marginBottom:6 }}>{ld.locName}</p>
                              {[
                                { label:'Stock',   val:ld.locStockBalance.toFixed(2) },
                                { label:'Margin',  val:`${ld.salesMargin}%` },
                                { label:'Retail',  val:`LKR ${ld.retailPrice.toFixed(2)}` },
                    ...
                    <Card id="sec-image" title="Item Image" icon={<IImage s={13}/>}>
                      <div style={{ display:'flex',alignItems:'center',gap:18 }}>
                        <div style={{ width:96,height:96,borderRadius:12,background:current.itemPic?'transparent':'#f3f6f6',border:'2px dashed #d1d9da',display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',flexShrink:0 }}>
                          {current.itemPic
                            ?<img src={current.itemPic} alt="item" style={{ width:'100%',height:'100%',objectFit:'cover' }}/>
                            :<div style={{ textAlign:'center',color:'#9ca3af' }}><IBox s={26}/><p style={{ fontSize:10,marginTop:4,fontWeight:600 }}>No Image</p></div>}
                        </div>
                        <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
                          <input ref={fileInputRef} type="file" accept="image/*" style={{ display:'none' }} onChange={handlePicChange}/>
                          <button className="btn-clear" onClick={()=>fileInputRef.current?.click()}><IImage s={14}/> Upload</button>
              ...
              <div style={{ background:'#dce8e8',borderTop:'1.5px solid rgba(30,58,64,0.12)',padding:'12px 16px',display:'flex',gap:10,flexShrink:0,flexWrap:'wrap',alignItems:'center',borderRadius:'0 0 14px 14px' }}>
                ...
                {activeTab==='details'&&(
                  <button className="btn-save" onClick={handleSave} disabled={busy}>
                    {saving?<><span className="spinner"/> Saving…</>:<><ISave s={14}/> {isNew?'Create Item':'Save Item'}</>}
                  </button>
                )}
